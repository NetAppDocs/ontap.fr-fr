...
.Mode actif
Pour le protocole ARP fonctionnant avec ONTAP 9.10.1 à 9.15.1, le protocole ARP passe en _active mode_ une fois l'intervalle d'apprentissage optimal terminé. Une fois que le protocole ARP est passé en mode actif, ONTAP crée des instantanés ARP pour protéger les données en cas de détection d'une menace.
