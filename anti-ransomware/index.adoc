...
[role="lead"]
À partir de ONTAP 9.10.1, les administrateurs ONTAP peuvent activer la protection autonome contre les ransomwares (ARP) pour effectuer une analyse de la charge de travail dans les environnements NAS (NFS et SMB) afin de détecter et d’avertir de manière proactive toute activité anormale pouvant indiquer une attaque par ransomware. À partir de ONTAP 9.17.1, ARP prend également en charge les volumes de périphériques à blocs, y compris les volumes SAN contenant des LUN ou des espaces de noms NVMe, ou les volumes NAS contenant des disques virtuels provenant d’hyperviseurs tels que VMware. À partir de ONTAP 9.17.1P5, les hyperviseurs Hyper-V, KVM et OpenStack sont également pris en charge.
