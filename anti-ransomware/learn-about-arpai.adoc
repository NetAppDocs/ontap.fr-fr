...
ARP/AI crée un instantané dès la détection des premiers signes d'une attaque. Une analyse est ensuite menée pour confirmer ou infirmer l'attaque potentielle. À partir d'ONTAP 9.17.1, ARP/AI génère également des instantanés toutes les 4 heures ; la présence de ces instantanés ne doit pas être considérée comme une anomalie. Si une attaque est confirmée, la probabilité d'attaque est définie sur  `Moderate` et une notification d'attaque est générée.
