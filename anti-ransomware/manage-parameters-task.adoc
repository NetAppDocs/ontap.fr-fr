...
NOTE: Pour réduire le nombre élevé de fausses alertes positives, accédez à *Stockage > Volumes > Sécurité > Configurer les caractéristiques de charge de travail* et désactivez *Surveiller les nouveaux types de fichiers*. Ce paramètre est désactivé par défaut dans ONTAP 9.14.1 P7, 9.15.1 P1, 9.16.1 et versions ultérieures. À partir d'ONTAP 9.18.1 P2, System Manager n'affiche plus les caractéristiques de charge de travail ARP, y compris les statistiques de référence et de pics. Les statistiques de pics restent disponibles via l'interface de ligne de commande ONTAP et l'API REST ONTAP pour une analyse avancée.
