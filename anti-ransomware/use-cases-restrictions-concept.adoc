...
[role="lead"]
La protection anti-ransomware autonome est disponible pour les charges de travail NAS à partir de ONTAP 9.10.1. Avant de déployer ARP, vous devez connaître les utilisations recommandées et les configurations prises en charge, ainsi que les implications en termes de performances.
...
| Volumes protégés avec SnapMirror asynchrone | X | X |  |  


| SVM protégé avec SnapMirror asynchrone | X | X |  |  


| Les SVM activés pour la migration des données | X | X |  |  


| Volumes FlexGroup | X |  |  |  


| Vérification multi-administrateurs | X |  |  |  
|===
...
* SnapMirror asynchrone (dans ONTAP 9.10.1 à 9.12.1) SnapMirror asynchrone est pris en charge à partir de ONTAP 9.12.1. Pour plus d'informations, voir <<snapmirror>>.)
* Volumes restreints
