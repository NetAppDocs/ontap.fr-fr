...
* À partir d'ONTAP 9.10.1 jusqu'à 9.15.1, les charges de travail NAS sur des volumes comportant plus de 4 000 extensions de fichiers uniques.
+
Le modèle ARP d'origine peut analyser jusqu'à 4 000 extensions de fichiers par volume. Si le nombre d'extensions de fichiers uniques sur un volume dépasse cette limite, ARP ne peut pas détecter efficacement l'activité des ransomwares et la charge de travail est considérée comme inadaptée à ARP. Cette limite ne s'applique pas à ARP/AI à partir d'ONTAP 9.16.1.
...
* L'état de configuration ARP et les instantanés ARP ne sont pas répliqués sur le volume secondaire dans le cadre de la synchronisation synchrone SnapMirror ou de la synchronisation active SnapMirror dans ONTAP 9.19.1.
...
=== SnapMirror relationships après la mise à niveau vers ONTAP 9.19.1
