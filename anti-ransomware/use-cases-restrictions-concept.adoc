...
* L'analyse ARP/AI s'exécute sur le volume actif en lecture-écriture dans les relations SAN SnapMirror synchrones et SnapMirror active sync. ARP crée des instantanés sur le volume principal et effectue une détection des ransomwares basée sur l'apprentissage automatique.
* Les instantanés ARP ne sont pas répliqués sur le volume secondaire dans les relations SnapMirror synchrones ou SnapMirror active sync. Vous devez link:enable-task.html["Réactiver ARP"] sur le volume de récupération après le basculement.
