...
* La valeur  `scan-queue-timeout` doit être inférieure à la valeur  `scanner-pool request-timeout`. Si elle est définie sur une valeur supérieure, l’accès aux fichiers pourrait être retardé et finir par expirer. Pour éviter cela, configurez la valeur  `scan-queue-timeout` à 5 secondes de moins que la valeur  `scanner-pool request-timeout`. Consultez la documentation du fournisseur du moteur d’analyse pour obtenir des instructions sur la modification des paramètres  `scan-queue-timeout`. La valeur  `scanner-pool timeout` peut être modifiée en utilisant la commande suivante en mode avancé et en fournissant la valeur appropriée pour le paramètre  `request-timeout` :
