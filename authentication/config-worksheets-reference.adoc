...
|===
Vous fournissez ces valeurs avec le `security certificate install` Commande lorsque vous installez un certificat numérique signé par une autorité de certification pour l'authentification du cluster ou d'un SVM en tant que serveur SSL. Seules les options pertinentes à ce guide sont présentées dans le tableau suivant.
...
====
Seules les options pertinentes pour ce guide sont indiquées dans le tableau suivant :
