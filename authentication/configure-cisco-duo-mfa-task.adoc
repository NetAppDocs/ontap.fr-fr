...
[NOTE]
====
Le paramètre `-is-enabled false` désactive l'authentification Duo, mais ne supprime pas la configuration Duo du chemin de connexion SSH. Dans certains environnements, les connexions SSH peuvent toujours nécessiter un flux d'authentification interactif, ce qui peut empêcher les connexions SSH non interactives. Pour rétablir le comportement de connexion SSH non interactive, supprimez complètement la configuration Duo avec `security login duo delete -vserver <STORAGE_VM_NAME>`.
