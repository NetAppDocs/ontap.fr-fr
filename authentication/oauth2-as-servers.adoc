...
Vous pouvez définir un serveur d'autorisation pour ONTAP à l'aide de n'importe quelle interface d'administration, notamment l'interface de ligne de commande ONTAP, System Manager ou l'API REST ONTAP. Par exemple, avec l'interface de ligne de commande ONTAP, vous utilisez la commande `security oauth2 client create`.
...
Les jetons d'accès OAuth 2.0 émis par les serveurs d'autorisation sont vérifiés par ONTAP et utilisés pour prendre des décisions d'accès basées sur les rôles pour les requêtes client de l'API REST ONTAP.



=== Acquisition d'un jeton d'accès

Vous devez obtenir un jeton d'accès auprès d'un serveur d'autorisation défini pour le cluster ONTAP où vous utilisez l'API REST ONTAP. Pour obtenir un jeton, vous devez contacter directement le serveur d'autorisation.
