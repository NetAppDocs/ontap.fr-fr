...
Vous devez obtenir un jeton d'accès à utiliser avec l'appel à l'API REST ONTAP. La demande de jeton est effectuée en dehors d'ONTAP et la procédure exacte dépend du serveur d'autorisation et de sa configuration. Vous pouvez demander le jeton via un navigateur web, avec une commande curl ou en utilisant un langage de programmation.
