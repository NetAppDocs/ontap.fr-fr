...
| Visualisation dans System Manager | X | X | X | X | X | X 


| Analyse de la capacité | X | X | X | X | X | X 


| Informations sur les données inactives | X | X | X | X | X | X 


| La prise en charge des volumes a migré depuis Data ONTAP 7-mode | X | X | X | X | X |  


| Personnalisation de la période inactive dans System Manager | X | X | X | X | X |  


| Suivi des activités au niveau du volume | X | X | X | X |  |  


| Téléchargez les données de suivi d'activité au format CSV | X | X | X | X |  |  


| Suivi d'activité au niveau de SVM | X | X | X |  |  |  


| De la chronologie | X | X | X |  |  |  


| Analyse de l'utilisation | X | X |  |  |  |  


| Option permettant d'activer l'analyse du système de fichiers par défaut | X |  |  |  |  |  
|===
