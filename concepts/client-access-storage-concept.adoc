...
---
= Découvrez l'authentification de clients et l'autorisation dans ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
ONTAP prend en charge Security Assertion Markup Language (SAML) pour l'authentification des utilisateurs distants. Plusieurs fournisseurs d'identité populaires (IdPs) sont pris en charge. Pour plus d'informations sur les IdPs pris en charge et les instructions pour activer l'authentification SAML, consultez link:../system-admin/configure-saml-authentication-task.html["Configurez l'authentification SAML"].
...
La prise en charge du framework Open Authorization (OAuth 2.0) est disponible à partir d'ONTAP 9.14. Vous pouvez uniquement utiliser OAuth 2.0 pour prendre des décisions d'autorisation et de contrôle d'accès lorsque le client utilise l'API REST ONTAP pour accéder à ONTAP. Cependant, vous pouvez configurer et activer cette fonctionnalité avec n'importe quelle interface d'administration ONTAP, y compris l'interface de ligne de commande ONTAP (CLI), System Manager et l'API REST ONTAP.
