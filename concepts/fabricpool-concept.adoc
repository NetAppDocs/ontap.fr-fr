...
summary: 'De nombreux NetApp clients possèdent d"importantes quantités de données stockées rarement consultées. On les appelle données froides. Ils possèdent également des données fréquemment consultées, que l"on appelle données chaudes. Idéalement, vous souhaitez conserver vos données chaudes sur votre stockage le plus rapide pour des performances optimales. Les données froides peuvent être déplacées vers un stockage plus lent, à condition qu"elles soient immédiatement disponibles en cas de besoin. Découvrez comment savoir quelles parties de vos données sont chaudes et lesquelles sont froides.' 
---
= Découvrez ONTAP FabricPool
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
link:../concept_cloud_overview.html["Document FabricPool System Manager"]
