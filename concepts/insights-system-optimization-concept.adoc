...
* Récupérez de l'espace en supprimant les snapshots.
...
*Recommandé* : activez le mode actif de protection anti-ransomware autonome pour ces volumes.
 ...
 a| 
La suppression automatique des snapshots est activée sur les volumes
 a| 
A besoin d'attention
 a| 
La suppression automatique des snapshots est activée sur un ou plusieurs volumes.
 a| 
*Recommandé* : désactivez la suppression automatique des instantanés. Sinon, en cas d'attaque par ransomware, il n'est pas toujours possible de restaurer les données de ces volumes.



 a| 
Les volumes ne disposent pas de règles de snapshot
 a| 
A besoin d'attention
 a| 
Une politique de snapshot adéquate n'est pas associée à un ou plusieurs volumes.
 a| 
*Recommandé* : attachez une stratégie de snapshot aux volumes qui n'en ont pas. Sinon, en cas d'attaque par ransomware, il n'est pas toujours possible de restaurer les données de ces volumes.
