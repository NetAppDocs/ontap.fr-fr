...
summary: 'Les performances des charges de travail commencent à être affectées par la latence lorsque la charge de travail sur un nœud dépasse les ressources disponibles. Vous pouvez gérer un nœud surchargé en augmentant les ressources disponibles, par exemple en mettant à niveau les disques ou le CPU, ou en réduisant la charge, par exemple en déplaçant des volumes ou des LUN vers d"autres nœuds selon les besoins.' 
---
= Découvrez l'équilibrage de charge dans ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
