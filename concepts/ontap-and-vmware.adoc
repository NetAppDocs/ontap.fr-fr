...
---
= Découvrez ONTAP et VMware vSphere
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: À partir d'ONTAP 9.15.1, vous pouvez créer une réservation persistante pour un volume virtuel à l'aide de SCSI-3. Cette fonctionnalité est uniquement prise en charge avec les ONTAP tools for VMware vSphere 9 et un cluster de basculement Windows Server (WSFC).
...
=== Récupération de site en direct VMware

VMware Live Site Recovery, anciennement connu sous le nom de Site Recovery Manager (SRM), offre une fonctionnalité de reprise après sinistre. Live Site Recovery s'intègre aux ONTAP tools for VMware vSphere pour accéder et exploiter la fonctionnalité de gestion des données ONTAP.
