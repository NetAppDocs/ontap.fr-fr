...
---
= Découvrez l'efficacité du stockage ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
L'efficacité du stockage sensible à la température (TSSE), introduite dans ONTAP 9.8, est activée automatiquement sur les volumes AFF nouvellement créés et à provisionnement fin. Il n'est pas activé surlink:builtin-storage-efficiency-concept.html["Plates-formes AFF A70, AFF A90 et AFF A1K"] qui sont introduites dans ONTAP 9.15.1, qui utilisent un processeur de déchargement matériel.




NOTE: TSSE n'est pas pris en charge uniquement sur les volumes à provisionnement épais.
