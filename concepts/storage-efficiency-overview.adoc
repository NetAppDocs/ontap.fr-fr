...
+
À partir de ONTAP 9.15.1, ONTAP assure la link:builtin-storage-efficiency-concept.html["Efficacité du stockage du processeur ou du processeur de déchargement dédié"] compaction et la compaction des données sur les plateformes AFF A70, AFF A90 et AFF A1K. L'efficacité du stockage est activée automatiquement, sans configuration.
