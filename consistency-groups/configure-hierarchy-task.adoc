...
[role="lead"]
Si la charge de travail de vos applications se compose de plusieurs sous-ensembles de volumes, chacun d'entre eux étant cohérent sur l'ensemble des volumes associés, ONTAP vous permet de créer un groupe de cohérence hiérarchique. Les groupes de cohérence hiérarchiques ont un parent qui peut inclure jusqu'à cinq groupes de cohérence individuels. Les groupes de cohérence hiérarchiques peuvent prendre en charge différentes règles Snapshot locales sur plusieurs groupes de cohérence ou volumes individuels. Si vous utilisez une stratégie SM-BC distante, celle-ci s'appliquera à l'ensemble du groupe de cohérence hiérarchique (parent et enfants).
