...
---
= En savoir plus sur les groupes de cohérence ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Pour plus d'informations sur l'API REST ONTAP, consultez  https://docs.netapp.com/us-en/ontap-automation/reference/api_reference.html#access-a-copy-of-the-ontap-rest-api-reference-documentation["Documentation de référence de l'API REST ONTAP"^].
...
Vous pouvez consulter les données de surveillance dans l’onglet **Overview** du menu du groupe de cohérence dans System Manager ou en les demandant dans l’API REST ONTAP. À partir d’ONTAP 9.14.1, vous pouvez consulter les métriques du groupe de cohérence avec l’interface de ligne de commande ONTAP à l’aide de la commande  `consistency-group metrics show`. En savoir plus sur  `consistency-group metrics show` dans la link:https://docs.netapp.com/us-en/ontap-cli/vserver-consistency-group-metrics-show.html["Référence de commande ONTAP"^].


NOTE: Dans ONTAP 9.13.1, vous ne pouvez récupérer les métriques historiques qu'en utilisant l'API REST ONTAP. À partir d'ONTAP 9.14.1, les métriques historiques sont également disponibles dans System Manager.
...
Les groupes de cohérence offrent une protection cohérente au niveau des applications, garantissant la cohérence de vos données sur plusieurs volumes ou LIF. Lors de la création d'un instantané d'un groupe de cohérence, une « barrière » est établie sur le groupe de cohérence. La barrière initie une file d'attente pour les E/S jusqu'à la fin de l'opération d'instantané, assurant ainsi la cohérence des données à un instant donné sur toutes les entités du groupe de cohérence. La barrière peut entraîner un pic de latence transitoire lors des opérations de création d'instantanés, telles qu'une stratégie d'instantané planifiée ou la création d'un instantané avec System Manager. Pour en savoir plus sur le contexte de l'API REST ONTAP et de l'interface de ligne de commande ONTAP, consultez la link:https://kb.netapp.com/on-prem/ontap/DM/REST-API["Documentation de l'API REST ONTAP"^] et la link:https://docs.netapp.com/us-en/ontap-cli/["Référence de commande ONTAP"^].
...
Les volumes et les instantanés associés aux groupes de cohérence placés sur des agrégats en miroir sont répliqués sur le site distant (site B). Le contenu des volumes sur le site B garantit l'ordre d'écriture du groupe de cohérence, ce qui vous permet de restaurer à partir du site B en cas de sinistre. Vous pouvez accéder aux instantanés des groupes de cohérence à l'aide du groupe de cohérence avec l'API REST ONTAP et System Manager sur les clusters exécutant ONTAP 9.11.1 ou une version ultérieure. À partir d'ONTAP 9.14.1, vous pouvez également accéder aux instantanés avec l'interface de ligne de commande ONTAP.
...
Les instantanés de groupe de cohérence créés dans l'API REST ONTAP peuvent être gérés via l'interface Groupe de cohérence de System Manager et via les points de terminaison de l'API REST ONTAP pour les groupes de cohérence. À partir de ONTAP 9.14.1, les instantanés de groupe de cohérence peuvent également être gérés avec l'interface de ligne de commande ONTAP (CLI).
