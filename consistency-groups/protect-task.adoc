...
* Lorsque vous lancez la création de l’instantané, vous pouvez définir une valeur de délai d’expiration facultative comprise entre 5 et 120 secondes. Si aucune valeur de délai d’expiration n’est fournie, l’opération expire après le délai par défaut de 7 secondes. Dans l’API REST ONTAP, définissez la valeur du délai d’expiration avec le paramètre  `action_timeout`. Dans l’interface de ligne de commande ONTAP, utilisez l’indicateur  `-timeout`.


.Étapes
Vous pouvez effectuer une capture instantanée en deux phases avec l'API REST ONTAP ou, à partir de ONTAP 9.14.1, l'interface de ligne de commande ONTAP. Cette opération n'est pas prise en charge dans System Manager.


NOTE: Si vous lancez la création d'un instantané avec l'API REST ONTAP, vous devez valider l'instantané avec l'API REST ONTAP. Si vous lancez la création d'un instantané avec l'interface de ligne de commande ONTAP (CLI), vous devez valider l'instantané avec l'interface de ligne de commande ONTAP. La combinaison des méthodes n'est pas prise en charge.
