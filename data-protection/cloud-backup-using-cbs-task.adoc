...
---
= Sauvegardez les données à l'aide du service de sauvegarde et de restauration BlueXP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
NOTE: À partir de ONTAP 9.10.1, vous pouvez choisir StorageGRID comme fournisseur de sauvegarde dans le cloud, mais uniquement si BlueXP  est déployé sur site. Le connecteur BlueXP doit être installé sur site et disponible via l'application BlueXP Software-as-a-service (SaaS).
...
* Les FlexGroups ne peuvent pas être sauvegardées avec la sauvegarde et la restauration BlueXP .
