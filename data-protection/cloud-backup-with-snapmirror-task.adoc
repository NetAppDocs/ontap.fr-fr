...
---
= Sauvegardez les données vers le cloud à l'aide de ONTAP SnapMirror cloud
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
* SnapMirror cloud augmente le nombre de compartiments de stockage d'objets S3 pris en charge par cluster, passant de 6 à 100. Cette limite s'applique au nombre total de stockages d'objets configurés sur l'ensemble des points de terminaison SnapMirror cloud object store d'un cluster ONTAP. Lors de l'utilisation de cette fonctionnalité, tenez compte des points suivants :
+
...
* SnapMirror cloud prend en charge les opérations de sauvegarde et de restauration pour les volumes FlexGroup sur les configurations MetroCluster en utilisant l’existant link:https://docs.netapp.com/us-en/ontap-automation/get-started/access_rest_api.html["Les API REST de ONTAP"^]. Cette fonctionnalité vous permet de créer des relations SnapMirror cloud pour les volumes FlexGroup sur les configurations MetroCluster qui sont maintenues depuis le site partenaire après un basculement et un retour arrière.


Avant d'utiliser la fonctionnalité cloud SnapMirror, vous devez demander une clé de licence API cloud SnapMirror auprès du site d'assistance NetApp : link:https://mysupport.netapp.com/site/tools/snapmirror-cloud-api-key["Demandez la clé de licence de l'API cloud SnapMirror"^].
