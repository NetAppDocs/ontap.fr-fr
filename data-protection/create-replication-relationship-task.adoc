...
NOTE: Le `schedule` Le paramètre n'est pas applicable lors de la création de relations SnapMirror synchrone.
...
NOTE: La règle prédéfinie `SnapCenterSync` est de type `Sync`. Cette règle réplique toute copie Snapshot créée avec le `snapmirror-label` de « cohérent_app ».
