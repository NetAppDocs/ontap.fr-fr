...
----
. Depuis le SVM source d'origine ou le cluster source d'origine, libérer la relation d'origine tout en conservant les copies Snapshot intactes :
+
...
----
. Depuis le SVM de destination d'origine ou le cluster de destination d'origine, relâcher la relation inversée tout en préservant l'intégrité des copies Snapshot :
+
`snapmirror release -source-path SVM: -destination-path SVM: -relationship-info-only true`
