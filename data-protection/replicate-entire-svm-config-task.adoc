...
* À partir d'ONTAP 9.19.1, l'activation d'IPsec n'est pas prise en charge sur les SVM configurées pour SVM DR. Si vous utilisez IPsec, vous devez le désactiver ou choisir une autre SVM pour configurer une relation SVM DR. Vous recevrez une erreur si vous utilisez ONTAP 9.19.1 ou une version ultérieure et que vous tentez d'activer IPsec sur une SVM configurée pour SVM DR.
...
. À partir du cluster source, créez une relation de pair SVM à l'aide de la commande  `vserver peer create`.
+
