...
** Elles ne sont pas supprimées lors des opérations de bascule et resynchronisation. Lorsque vous configurez une relation SVM DR à l'aide de la règle mirror-vault en utilisant ONTAP 9.9.1 et versions ultérieures, la politique se comporte comme suit :
...
** L'association de volumes aux règles Snapshot définies par l'utilisateur et par le système ne sont pas copiées vers la destination. + SVM.
...
[NOTE]
====
L'indépendance de version n'est pas prise en charge pour la réplication des SVM. Dans une configuration SVM DR, le SVM de destination doit se trouver sur un cluster exécutant la même version ONTAP que le cluster SVM source pour prendre en charge les opérations de basculement et de retour arrière.
...
MetroCluster
 a| 
Depuis ONTAP 9.11.1, les deux côtés d'une relation de SVM DR dans une configuration MetroCluster peuvent servir de source pour d'autres configurations de reprise après incident SVM.

Depuis ONTAP 9.5, la réplication de SnapMirror SVM est prise en charge dans les configurations MetroCluster.

* Une configuration MetroCluster ne peut pas être la destination d'une relation de SVM DR.
* Seul un SVM actif au sein d'une configuration MetroCluster peut être à la source d'une relation SVM DR.
+
...
* Lorsqu'une configuration MetroCluster est dans un état stable, le SVM MetroCluster destination-sync ne peut pas être à la source d'une relation SVM DR, car les volumes ne sont pas en ligne.
...
SnapMirror synchrone
 a| 
Non pris en charge avec SVM DR.

|===


== Configurations répliquées dans les relations de reprise après incident des SVM

Le tableau suivant montre l'interaction du `snapmirror create `-identity-preserve` et le `snapmirror policy create` option -rebuter-configs réseau :
