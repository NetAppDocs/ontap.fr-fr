...
| Réplication Snapshot créée par les applications | ONTAP 9.7 | Si un instantané est étiqueté avec le libellé approprié au moment de l’ `snapshot create`opération, en utilisant la CLI ou l’API ONTAP, SnapMirror synchrone réplique les instantanés, qu’ils soient créés par l’utilisateur ou par des scripts externes, après la mise en veille des applications. À partir de ONTAP 9.14.1, l’exigence de libellé pour la réplication des instantanés créés par les applications n’existe plus. Pour plus d’informations sur la réplication des instantanés créés par les applications, consultez la link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/SnapMirror/How_to_replicate_application_created_snapshots_with_SnapMirror_Synchronous["Base de connaissances NetApp : Comment répliquer des snapshots créés par une application avec SnapMirror synchrone"^]. 


| Protection anti-ransomware autonome | ONTAP 9.19.1 |  
...
| Restauration NDMP/NDMP | ONTAP 9.13.1 | Les clusters source et de destination doivent exécuter ONTAP 9.13.1 ou une version ultérieure pour effectuer une sauvegarde NDMP basée sur le vidage ou SMTape des volumes synchrones SnapMirror. Les restaurations NDMP, qu'elles soient basées sur le vidage ou SMTape, ne sont pas prises en charge pour les volumes de destination dans une relation SnapMirror active sync ou SnapMirror synchrone. 
...
| Réplication des opérations de gestion des instantanés | ONTAP 9.19.1 | À partir d'ONTAP 9.19.1, les opérations de gestion des instantanés `snapshot delete`, `snapshot rename` et `snapshot modify` sont répliquées sur le cluster secondaire. 


| Réplication d'instantané planifiée | ONTAP 9.19.1 | À partir de ONTAP 9.19.1, vous pouvez activer la réplication des instantanés planifiés à l'aide d'une stratégie SnapMirror. Dans les versions antérieures à ONTAP 9.19.1, les instantanés planifiés créés à l'aide d'une stratégie d'instantané ne sont pas répliqués. 
...
| Reprise d'activité de SVM | ONTAP 9.6  a| 
* Une source SnapMirror synchrone peut également être une source de reprise après sinistre SVM, par exemple, une configuration en éventail avec SnapMirror synchrone comme une branche et la reprise après sinistre SVM comme l'autre.
* Une source SnapMirror synchrone ne peut pas être une destination de reprise après sinistre SVM, car SnapMirror synchrone ne prend pas en charge la mise en cascade d'une source de protection des données. Vous devez libérer la relation synchrone avant d'effectuer une resynchronisation de basculement de reprise après sinistre SVM dans le cluster de destination.
* Une destination SnapMirror synchrone ne peut pas servir de source de reprise après sinistre SVM, car la reprise après sinistre SVM ne prend pas en charge la réplication des volumes DP. Une resynchronisation inversée de la source synchrone entraînerait l'exclusion du volume DP dans le cluster de destination par la reprise après sinistre SVM.




| link:../snaplock/snapshot-lock-concept.html["Des snapshots inviolables"] | ONTAP 9.19.1 | Vous pouvez spécifier la durée de validité uniquement lors de la création d'un instantané. Il est impossible de définir ou de prolonger la durée de validité des instantanés existants. 
...
* SnapCenter
* Volumes SnapLock
* Restauration NDMP basée sur Dump ou SMTape vers des volumes de destination dans une relation de synchronisation active SnapMirror ou synchrone SnapMirror
...
== Réplication d'instantané planifiée

À partir d'ONTAP 9.19.1, vous pouvez utiliser le paramètre de politique SnapMirror `-sync-snapshot-replication-mode` en mode de privilège avancé pour répliquer les snapshots programmés vers la destination SnapMirror. La réplication des snapshots programmés fournit une symétrie entre les volumes primaire et secondaire dans une relation synchrone SnapsMirror.

.Prérequis
* Vous devez être administrateur de cluster ou de SVM
* Vous devez être en mode de privilège avancé
* Vous devez configurer une stratégie SnapMirror définie par l'utilisateur qui utilise le paramètre  `-sync-snapshot-replication-mode` et inclut l'option  `sched_created`.
+
** Les politiques définies par le système ne peuvent pas être modifiées. Vous devez créer une nouvelle politique et rétablir la relation.
** Les modifications de stratégie définies par l'utilisateur nécessitent une resynchronisation (mise en veille suivie d'une reprise) pour prendre effet sur les relations existantes.




Ce paramètre accepte les options suivantes :

* `none`- désactive la réplication des instantanés synchrones non-SnapMirror
* `application_created`- réplique les instantanés créés par l'application
* `sched_created` - réplique les instantanés planifiés
* `sm_created` - réplique les instantanés créés par SnapMirror de façon asynchrone (branche de diffusion)
* Une liste de types séparés par des virgules : `application_created`, `sched_created`, et `sm_created`.


Ce paramètre n'entre en vigueur qu'une fois l'état de la relation  `InSync`. Les types d'instantanés non répertoriés sont stockés uniquement sur le volume principal.

À partir d'ONTAP 9.19.1, `snapmirror initialize` tous les instantanés sont répliqués sur le cluster secondaire, tandis que  `snapmirror resync` seuls les nouveaux instantanés créés après le dernier instantané commun sont répliqués. Lors d'une opération de resynchronisation, les instantanés non répliqués marqués comme primaire uniquement lorsque l'état de la relation est  `InSync` ne sont pas répliqués sur le cluster secondaire, et ces instantanés sont conservés sur le nouveau cluster secondaire après une opération de resynchronisation inverse.


NOTE: La création d'instantanés sur SnapMirror synchrones volumes peut augmenter la latence d'E/S des utilisateurs. Il est recommandé d'utiliser des planifications d'instantanés avec une fréquence supérieure à 30 minutes.

.Informations associées
* https://www.netapp.com/pdf.html?item=/media/17174-tr4733.pdf["Rapport technique NetApp 4733 : configuration synchrone et bonnes pratiques SnapMirror"^]
* link:https://docs.netapp.com/us-en/ontap-cli//volume-snapshot-create.html["créer un instantané de volume"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-delete.html["suppression de l'instantané de volume"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-modify.html["modifier l'instantané de volume"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-rename.html["renommer l'instantané de volume"^]
