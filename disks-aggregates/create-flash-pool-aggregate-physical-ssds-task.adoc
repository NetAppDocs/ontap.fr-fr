...
summary: 'Pour créer un niveau local Flash Pool, activez la fonctionnalité sur un niveau local existant composé de groupes RAID HDD, puis ajoutez un ou plusieurs groupes RAID SSD à ce niveau local. Vous obtenez ainsi deux ensembles de groupes RAID pour ce niveau local : les groupes RAID SSD (ou cache SSD) et les groupes RAID HDD.' 
---
