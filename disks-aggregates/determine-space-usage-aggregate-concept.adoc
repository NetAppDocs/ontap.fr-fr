...
* *Réserve snapshot*
+
Quantité d'espace réservée pour les snapshots d'agrégats, en fonction de la taille du volume. Elle est considérée comme un espace utilisé et n'est pas disponible pour le volume, l'agrégat des données ou des métadonnées.

* *Réserve snapshot inutilisable*
+
Quantité d'espace initialement allouée à la réserve Snapshot de l'agrégat qui n'est pas disponible pour les snapshots d'agrégat car elle est utilisée par les volumes associés à l'agrégat. Peut se produire uniquement pour les agrégats avec une réserve Snapshot d'agrégat non nulle.

* *Total utilisé*
+
Somme de tout l'espace utilisé ou réservé dans l'agrégat par les volumes, les métadonnées ou les snapshots.

* *Physique totale utilisée*
+
Quantité d'espace utilisée pour les données actuellement (au lieu d'être exclusivement réservée à une utilisation ultérieure). Inclut l'espace utilisé par les snapshots d'agrégats.



L'exemple suivant montre le `aggregate show-space` résultat de la commande pour un agrégat dont la réserve Snapshot est de 5 %. Si la réserve d'instantanés était 0, la ligne ne s'affiche pas.
