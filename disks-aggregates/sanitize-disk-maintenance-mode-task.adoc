...
+
`-p pattern1 -p pattern2 -p pattern3` spécifie un cycle de un à trois octets hexadécimaux définis par l'utilisateur
remplacer les motifs qui peuvent être appliqués successivement aux disques en cours de nettoyage. La valeur par défaut
le motif est constitué de trois passes, avec 0x55 pour le premier passage, 0xaa pour le second passage et 0x3c pour le
troisième passe.
