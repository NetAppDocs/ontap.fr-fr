...
. Modifiez le serveur de clé primaire pour ajouter des serveurs de clé secondaires. Le `-secondary-key-servers` Ce paramètre accepte une liste de trois serveurs clés maximum, séparés par des virgules. N'indiquez pas de numéro de port pour les serveurs de clés secondaires.
...
--
. Modifiez le serveur de clé primaire pour ajouter des serveurs de clé secondaires. Le `-secondary-key-servers` Ce paramètre accepte une liste de trois serveurs clés maximum, séparés par des virgules. N'indiquez pas de numéro de port pour les serveurs de clés secondaires.
`security key-manager external modify-server -vserver _svm_name_ -key-servers _primary_key_server_ -secondary-key-servers _list_of_key_servers_` Pour plus d'informations sur les serveurs de clés secondaires, consultez <<mod-secondary>>.
...
Les serveurs de clés secondaires sont gérés avec le `-secondary-key-servers` paramètre du `security key-manager external modify-server` commande. Le `-secondary-key-servers` Ce paramètre accepte une liste séparée par des virgules. L'ordre spécifié des serveurs de clés secondaires dans la liste détermine la séquence d'accès pour ces serveurs. L'ordre d'accès peut être modifié en exécutant la commande `security key-manager external modify-server` avec les serveurs de clés secondaires saisis dans un ordre différent. N'indiquez pas de numéro de port pour les serveurs de clés secondaires.
