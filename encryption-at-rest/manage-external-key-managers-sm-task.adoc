...
Le gestionnaire de clés intégré stocke et gère les clés dans une base de données sécurisée qui est interne au cluster. Son périmètre est le cluster. Un gestionnaire de clés externe stocke et gère les clés en dehors du cluster. Son périmètre peut être le cluster ou la machine virtuelle de stockage.

.Description de la tâche
Vous devez utiliser l'interface de ligne de commandes ONTAP (CLI) pour configurer les gestionnaires de clés externes pour les configurations MetroCluster. En savoir plus link:configure-external-key-management-concept.html["Configuration de la gestion externe des clés ONTAP à l'aide de l'interface de ligne de commande (CLI)"].

Un ou plusieurs gestionnaires de clés externes peuvent être utilisés. Les conditions suivantes s'appliquent :
