...
Vous devez utiliser l'interface de ligne de commandes ONTAP (CLI) pour configurer les gestionnaires de clés externes pour les configurations MetroCluster. En savoir plus link:configure-external-key-management-concept.html["${post_edited_translations.segment}"].
