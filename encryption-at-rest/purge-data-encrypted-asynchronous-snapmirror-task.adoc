...
summary: 'À partir d" ONTAP 9.8, vous pouvez utiliser secure-purge pour nettoyer sans interruption les données sur les volumes compatibles NVE avec une relation asynchrone SnapMirror .' 
---
...
+
.. Créez un instantané sur le volume source dans la relation asynchrone SnapMirror :
+
`volume snapshot create -snapshot snapshot_name -vserver _SVM_name_ -volume _volume_name_`

.. Mettez à jour la relation SnapMirror pour avancer l'instantané de base vers la destination :
+
