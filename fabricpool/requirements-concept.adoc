...
+
FabricPool ne prend pas en charge la connexion d'un Tier cloud à un agrégat contenant des volumes avec une garantie d'espace autre que `none`. Par exemple, un volume utilisant une garantie d'espace de `volume` (`-space-guarantee` `volume`) n'est pas pris en charge.
