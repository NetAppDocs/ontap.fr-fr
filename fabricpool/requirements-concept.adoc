...
+
** Le `auto` link:tiering-policies-concept.html["règle de hiérarchisation"]
...
FabricPool nécessite une licence basée sur la capacité lors de l'ajout de fournisseurs de stockage d'objets tiers (tels qu'Amazon S3) en tant que niveaux cloud pour les systèmes AFF et FAS.
