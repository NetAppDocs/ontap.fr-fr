...
---
= À propos des règles de Tiering FabricPool dans ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
====
Vous pouvez utiliser[`volume object-store tiering show` la commande pour afficher l'état de la hiérarchisation d'un volume FabricPool. Pour en savoir plus sur la commande link:https://docs.NetApp.com/US-en/ONTAP-cli//volume-object-store-tiering-show.html[`volume object-store tiering show`], consultez le document de référence sur les commandes ONTAP.
