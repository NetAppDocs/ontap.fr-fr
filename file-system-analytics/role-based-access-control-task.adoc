...
[role="lead"]
À partir de ONTAP 9.12.1, ONTAP inclut un rôle prédéfini de contrôle d'accès basé sur les rôles (RBAC) appelé `admin-no-fsa`. Le rôle  `admin-no-fsa` accorde des droits d'administration, mais empêche l'utilisateur d'effectuer des opérations liées au point de terminaison  `files` (c'est-à-dire File System Analytics) dans l'interface de ligne de commande ONTAP, l'API REST ONTAP et dans System Manager.
