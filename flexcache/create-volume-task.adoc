...
À partir de ONTAP 9.5, FlexCache prend en charge les volumes FlexVol en tant que volumes d'origine et les volumes FlexGroup en tant que volumes FlexCache.  Depuis ONTAP 9.7, les volumes FlexVol et FlexGroup sont pris en charge en tant que volumes d'origine.

.Avant de commencer
* Vous devez exécuter ONTAP 9.5 ou une version ultérieure
* Si vous utilisez ONTAP 9.6 ou une version antérieure, vous devez link:add-license-task.html["Ajoutez une licence FlexCache"].
+
Aucune licence FlexCache n'est requise pour ONTAP 9.7 ou version ultérieure.
