...
.Avant de commencer
Si la fonction d'écriture différée FlexCache est activée, vous devez la désactiver avant de pouvoir supprimer un volume FlexCache. Voir link:../flexcache-writeback/flexcache-writeback-enable-task.html#disable-write-back-on-a-flexcache-volume["Désactiver l'écriture différée sur un volume FlexCache"].
