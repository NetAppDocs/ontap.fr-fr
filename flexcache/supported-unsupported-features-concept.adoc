...
NOTE: Vous pouvez créer un volume FlexCache comme cache pour un volume d'origine dont la hiérarchisation FabricPool est activée, mais le volume FlexCache lui-même ne peut pas être hiérarchisé.
...
Prise en charge à partir de ONTAP 9.18.1
...
Non
 ...
 a| 
SnapMirror relations asynchrones
 a| 
Oui.

En savoir plus sur les origines de FlexCache® :
...
Prise en charge à partir d' ONTAP 9.5. Le SVM principal d'une relation SVM DR peut avoir le volume d'origine ; cependant, si vous utilisez une version ONTAP antérieure à ONTAP 9.18.1, lorsque la relation SVM DR est interrompue, la relation FlexCache doit être recréée avec un nouveau volume d'origine.

À partir d' ONTAP 9.18.1, lorsqu'un SVM d'origine bascule, les caches basculent automatiquement vers l'origine sur le site de reprise après sinistre. Les étapes de récupération manuelle sont éliminées.

xref:create-volume-task.adoc[Découvrez comment créer des volumes FlexCache .]
 a| 
Non

Vous pouvez avoir des volumes d'origine FlexCache dans les SVM primaires, mais pas dans les SVM secondaires. La relation FlexCache n'est pas répliquée, mais les données du volume d'origine sont répliquées comme un volume classique.
...
NOTE: Dans les versions d'ONTAP 9 antérieures à 9.5, les volumes FlexVol d'origine peuvent uniquement fournir des données aux volumes FlexCache créés sur des systèmes exécutant Data ONTAP 8.2.x en mode 7-Mode. À partir d'ONTAP 9.5, les volumes FlexVol d'origine peuvent également fournir des données aux volumes FlexCache sur les systèmes ONTAP 9. Pour plus d'informations sur la migration de FlexCache 7-Mode vers FlexCache ONTAP 9, consultez link:https://www.netapp.com/pdf.html?item=/media/7336-tr4743.pdf["Rapport technique NetApp 4743 : FlexCache in ONTAP"^].
