...
.. Définissez une règle pour la politique de SnapVault qui détermine les copies Snapshot transférées pendant les opérations d'initialisation et de mise à jour : `snapmirror policy add-rule -vserver svm_name -policy policy_for_rule - snapmirror-label snapmirror-label -keep retention_count -schedule schedule`
