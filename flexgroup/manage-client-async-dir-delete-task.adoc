...
Les droits sont accordés par volume.

Vous ne pouvez déplacer que des répertoires.  Vous ne pouvez pas déplacer de fichiers vers le répertoire .ontaptrashbin.

link:fast-directory-delete-asynchronous-task.html#delete-directories-asynchronously["Découvrez comment utiliser ONTAP pour supprimer des répertoires de manière asynchrone à partir des volumes FlexVol et FlexGroup"].
...
.Étapes
. Depuis l'interface de ligne de commande du cluster, passez en mode de privilèges avancés : `-privilege advanced`
