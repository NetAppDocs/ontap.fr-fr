...
+
Le rééquilibrage crée des inodes partitionnés qui, en grand nombre, peuvent dégrader les performances. Dans le cadre d'une conversion, on estime que 50 à 85 % des données d'un volume FlexVol sont déplacées vers un nouveau composant, ce qui entraîne la création d'un grand nombre d'inodes partitionnés pour ces fichiers.
