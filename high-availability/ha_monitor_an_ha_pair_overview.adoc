...
Vous devez définir la valeur sur ha pour que le nœud puisse participer à une configuration de basculement du stockage (paire HA).
Le `non-ha` la valeur est utilisée uniquement dans une configuration de cluster autonome ou à un seul nœud. | `storage failover show -fields mode` 
