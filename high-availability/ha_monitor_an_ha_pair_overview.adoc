...
| Afficher les nœuds sur lesquels le paramètre de basculement de stockage HA est activé, vous devez définir la valeur sur HA pour que le nœud participe à une configuration de basculement de stockage (paire HA). Le `non-ha` la valeur est utilisée uniquement dans une configuration de cluster autonome ou à un seul nœud. | `storage failover show -fields mode` 
...
| L'état de haute disponibilité des composants d'une paire haute disponibilité (sur les systèmes qui utilisent l'état HA) | `ha‑config show`Il s'agit d'une commande du mode maintenance. 
|===
...
La relocalisation de la propriété des agrégats SFO du nœud local vers le nœud de prise en charge est en cours. Le système attend que les opérations de mise à jour du firmware du disque en arrière-plan soient effectuées sur le nœud partenaire.
 ...
 a| 
Connecté à Partner_name. La tentative précédente de basculement a été abandonnée en raison de cette raison. Le nœud local possède certains agrégats SFO du partenaire. Réémettez un basculement du partenaire avec le `‑bypass-optimization` paramètre défini sur true pour le basculement des agrégats restants ou du rétablissement d'un partenaire pour le retour des agrégats transférés.
 a| 
...
Échec du retour précédent dans le module : nom du module. Le rétablissement automatique est lancé en quelques secondes.
 a| 
La tentative de retour précédente a échoué dans le module nom_module. Le rétablissement automatique est lancé en quelques secondes.
