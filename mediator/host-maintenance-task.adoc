...
[role="lead"]
Assurez-vous que le système d'exploitation hôte d' ONTAP Mediator est maintenu régulièrement afin d'obtenir des performances optimales.
...
Redémarrez l’hôte uniquement lorsque les clusters sont sains. Les clusters ne peuvent pas répondre aux échecs lorsque ONTAP Mediator est hors ligne. Planifiez une heure pour la maintenance avant le redémarrage.
...
== Mettre à niveau le noyau du système d'exploitation hôte

SCST doit être compilé pour le noyau que vous utilisez.  Pour mettre à jour le système d'exploitation, vous devez planifier une période de maintenance.


NOTE: Si vous avez installé ONTAP Mediator 1.12 ou une version ultérieure prenant uniquement en charge HTTPS, le paquet SCST n'est pas installé et le noyau du système d'exploitation hôte peut être mis à niveau sans incidence sur ONTAP Mediator. Consultez la documentation de votre système d'exploitation hôte pour obtenir les instructions de mise à niveau.

.Étapes
Suivez ces étapes pour mettre à niveau le noyau du système d'exploitation hôte.
...
* Effectuez cette étape avant d’installer ONTAP Mediator.  Si la version SCST que vous utilisez est plus récente que la version fournie avec le programme d'installation ONTAP Mediator, le programme d'installation ignore cette étape.
* Dans certains scénarios, l'installation par défaut de SCST sur le système d'exploitation mis à niveau peut échouer car le système d'exploitation mis à niveau nécessite un correctif.
+
À partir d'ONTAP Mediator 1.12.1, si l'installation échoue, vous pouvez utiliser l'outil de réparation SCST situé dans  `/usr/local/bin/mediator_repair_scst` pour réparer l'installation.

+
Si le système d'exploitation mis à niveau nécessite un correctif, l'outil le contient pour la version de SCST fournie avec le package ONTAP Mediator. L' `mediator_repair_scst`outil est un outil autonome distinct du programme d'installation principal et est inclus dans le fichier ONTAP Mediator `tgz`. Il n'affecte pas l'installation ni les chemins de mise à niveau d'ONTAP Mediator.
...
* Ne faites cela que si les certificats auto-signés ne sont plus valides parce que le nom d'hôte ou l'adresse IP change après l'installation ONTAP Mediator.
