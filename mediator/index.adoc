...
* Vous pouvez utiliser n'importe quelle version d'ONTAP Mediator pour surveiller une configuration IP MetroCluster.
...
Veuillez prendre en compte ces points avant de mettre à niveau ou d'installer ONTAP Mediator.
...
* Lorsque vous effectuez une mise à niveau vers une nouvelle version d'ONTAP Mediator, le programme d'installation met automatiquement à niveau vers la version SCST recommandée, sauf si une version supérieure est disponible. Pour obtenir des instructions sur l'installation manuelle d'une version SCST supérieure, consultez link:manage-task.html["Gérer le médiateur ONTAP"]. Pour les versions prises en charge, consultez le link:whats-new-concept.html#scst-support-matrix["Matrice de prise en charge SCST"].
...
Assurez-vous que les ports suivants sont disponibles et non utilisés pour ONTAP Mediator :
...
|===
Pour les clients SMBC, ONTAP ne nécessite pas que le port 3260 soit activé ou connecté.

* Si vous utilisez un pare-feu tiers, consultez link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/concept_mediator_requirements.html#firewall-requirements-for-ontap-mediator["Exigences relatives au pare-feu pour ONTAP Mediator"^].
...
=== Considérations relatives à la mise à niveau du système d'exploitation et à la compatibilité du noyau
...
* Ne mettez pas à niveau le noyau au-delà de la version du système d'exploitation prise en charge pour votre version d'ONTAP Mediator, car le module SCST testé risque de ne pas fonctionner.
