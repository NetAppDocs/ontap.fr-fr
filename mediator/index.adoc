...
* Dans ONTAP Mediator, le paquet SCST est utilisé pour fournir iSCSI pour la communication dans les configurations IP MetroCluster. Le paquet SCST est un module noyau compilé lors de l'installation. À partir d'ONTAP Mediator 1.12, vous pouvez configurer un ONTAP Mediator qui utilise HTTPS au lieu d'iSCSI pour surveiller une configuration IP MetroCluster.
