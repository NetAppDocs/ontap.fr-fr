...
summary: 'Pour installer ou mettre à niveau ONTAP Mediator, vous devez obtenir le package d"installation et exécuter le programme d"installation sur l"hôte.' 
---
...
* À partir d'ONTAP 9.19.1 et ONTAP Mediator 1.12, vous avez la possibilité d'installer ONTAP Mediator avec la prise en charge d'iSCSI et d'HTTPS, ou uniquement avec la prise en charge d'HTTPS. Si vous choisissez d'installer ONTAP Mediator avec la prise en charge d'HTTPS uniquement, le package SCST n'est pas installé.
...
* Lors de l'installation ou de la mise à niveau vers ONTAP Mediator 1.12 ou une version ultérieure, le programme d'installation vous invite à répondre aux éléments suivants :
+
...
+
** Répondez  `Yes` si vous prévoyez d'utiliser ONTAP Mediator configuré avec iSCSI pour surveiller une ou plusieurs configurations IP MetroCluster.
** Répondez  `No` si vous prévoyez d'utiliser ONTAP Mediator uniquement pour la synchronisation active SnapMirror, ou si vous prévoyez de configurer ONTAP Mediator avec HTTPS pour surveiller une ou plusieurs configurations IP MetroCluster.
+
Vous pouvez également utiliser l’ `--https-only`option dans la `./ontap-mediator-1.12.1/ontap-mediator-1.12.1`commande pour spécifier uniquement HTTPS.



* Pour les cas d'utilisation d'automatisation, vous pouvez utiliser la commande  `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 -y` pour ignorer les invites de l'installateur.
...
[NOTE]
====
Pour ignorer la vérification de signature lors de l'installation, utilisez cette commande : `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 --skip-code-signature-check`

====
+
Le programme d'installation crée les comptes nécessaires et installe les packages requis.  Si le médiateur est déjà installé, il vous invite à le mettre à jour.



.Exemple d'installation d'ONTAP Mediator avec prise en charge HTTPS et iSCSI (sortie console)
[%collapsible]
====
[listing]
----
...
----
====
.Exemple de mise à niveau d'ONTAP Mediator avec prise en charge HTTPS et iSCSI (sortie console)
[%collapsible]
====
[listing]
----
...
----
====
.Exemple d'installation d'ONTAP Mediator avec prise en charge de HTTPS uniquement (sortie console)
[%collapsible]
====
[listing]
----
...
----
====
.Exemple de mise à niveau d'ONTAP Mediator avec prise en charge de HTTPS uniquement (sortie console)
[%collapsible]
====
[listing]
----
...
== Enregistrez la clé de sécurité pour le démarrage sécurisé UEFI

À partir d' ONTAP Mediator 1.4, le mécanisme de démarrage sécurisé est activé sur les systèmes UEFI.  Lorsque le démarrage sécurisé est activé, vous devez effectuer des étapes supplémentaires pour enregistrer la clé de sécurité après l'installation.


NOTE: Si vous avez installé ONTAP Mediator 1.12 ou une version ultérieure prenant uniquement en charge HTTPS, le package SCST n'est pas installé et vous pouvez ignorer cette tâche.
...
NOTE: Après l'installation, la sortie système affiche les fichiers README et l'emplacement de la clé.
...
[NOTE]
====
Vous pouvez laisser la clé privée à son emplacement par défaut ou la déplacer vers un emplacement sécurisé.  Vous devez conserver la clé publique à son emplacement actuel afin que le gestionnaire de démarrage puisse l'utiliser.  Pour plus d'informations, consultez le fichier README.module-signing :

`[root@hostname ~]# ls /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/
README.module-signing  scst_module_key.der  scst_module_key.priv`

====
. Redémarrez l'hôte et utilisez le gestionnaire de démarrage UEFI de votre appareil pour approuver le nouveau MOK.  Vous aurez besoin de la phrase secrète fournie pour le `mokutil` utilité danslink:index.html#install-ontap-mediator-when-uefi-secure-boot-is-enabled["Installez le médiateur ONTAP lorsque le démarrage sécurisé UEFI est activé"] .




[[signing-scst-kernel-modules]]
== Signature des modules noyau SCST

Après l'installation ONTAP Mediator, si l'état de systemctl `mediator-scst` Le module apparaît comme ayant échoué (inactif). Suivez ces étapes pour signer le module noyau SCST.


NOTE: Si vous avez installé ONTAP Mediator 1.12 ou une version ultérieure prenant uniquement en charge HTTPS, le package SCST n'est pas installé et vous pouvez ignorer cette tâche.

.Étapes
. Au cours du processus de construction, une paire de clés publique/privée est générée dans le `/opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/` répertoire, en utilisant la commande suivante :
+
`[root@mediator-host ~]# ls /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/
README.module-signing  scst_module_key.der  scst_module_key.priv
[root@mediator-host ~]#`

. Démarrez le processus d'importation de la clé publique dans le référentiel de clés UEFI en exécutant les commandes suivantes :
+
...
. Le logiciel mokutil demande un mot de passe temporaire pour cette clé lors du processus d'importation.
. Vérifiez si le processus d'importation a commencé avec le `mokutil --list-new` puis redémarrez le système.  Le chargeur de démarrage lance le gestionnaire EFI MOK.
. Utilisez les menus à l'écran pour activer la clé du module noyau SCST.  Après le démarrage, exécutez `systemctl status mediator-scst` .  Une fois le service démarré, les modules du noyau SCST sont signés.
