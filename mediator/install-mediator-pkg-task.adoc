...
[CAUTION]
====
* Dans les configurations IP MetroCluster, lorsque ONTAP Mediator est configuré avec le protocole HTTPS, une seule instance d'ONTAP Mediator ne peut surveiller qu'une seule configuration IP MetroCluster. La surveillance simultanée de plusieurs configurations IP MetroCluster n'est pas prise en charge.
...
** Répondez `No` si vous prévoyez d'utiliser ONTAP Mediator uniquement pour la synchronisation active SnapMirror, ou si vous prévoyez de configurer ONTAP Mediator avec HTTPS pour surveiller une seule configuration IP MetroCluster par instance ONTAP Mediator.
+
Vous pouvez également utiliser l’ `--https-only`option dans la `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y`commande pour spécifier uniquement HTTPS.
...
[NOTE]
====
Pour ignorer la vérification de signature lors de l'installation, utilisez cette commande : `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y --skip-code-signature-check`
