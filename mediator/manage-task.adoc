...
[NOTE]
====
* Dans ONTAP Mediator 1.8 et les versions antérieures, les fichiers CA utilisés sont : `ca.key`, `ca.csr`, `ca.srl` et `ca.crt`. À partir de ONTAP Mediator 1.9, les fichiers requis sont `intermediate.key`, , `intermediate.csr`, `intermediate.srl` et `intermediate.crt`.
