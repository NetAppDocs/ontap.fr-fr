...
Vous pouvez remplacer les certificats auto-signés par des certificats tiers approuvés.
...
+
L'autorité de l'ICP vérifie la demande et signe le `.csr`, génération du certificat `ca.crt`.

+

NOTE: Pour les clusters SnapMirror Business Continuity (SM-BC), vous devez ajouter le certificat `ca.crt` À un cluster ONTAP. Voir link:https://docs.netapp.com/us-en/ontap/smbc/smbc_install_confirm_ontap_cluster.html#ontap-mediator["Configurer le médiateur ONTAP et les clusters pour SM-BC"].
...
.Étape
Pour générer un certificat de serveur, exécutez les commandes suivantes à partir du dossier `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config`:
...
`-CAcreateserial` est utilisée pour générer le fichier `ca.srl`.
...
.Étapes
. Remplacer l' `ca.*` avec les certificats tiers.
...
.Étapes
. Configurez le fichier `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config/ontap_mediator.user_config.yaml` pour remplacer les valeurs de variable par défaut dans `ontap_mediator.config.yaml` fichier.
+
Par exemple, si vous obtenez `intermediate.crt` À partir de l'autorité PKI et stockez sa clé privée `intermediate.key` et de la demande de signature de certificat `intermediate.csr` à un emplacement `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config`, le fichier user_config doit ressembler à ce qui suit :
