...
ONTAP Mediator utilise SCST pour fournir iSCSI (voir  http://scst.sourceforge.net/index.html[]). Ce package est un module noyau qui est compilé lors de l'installation spécifiquement pour le noyau. Toute mise à jour du noyau peut nécessiter la réinstallation de SCST. Vous pouvez également désinstaller puis réinstaller ONTAP Mediator, puis reconfigurer la relation ONTAP.
