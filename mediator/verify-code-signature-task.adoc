...
[role="lead"]
NetApp recommande de vérifier la signature de code du Médiateur ONTAP avant d'installer le progiciel d'installation du Mediator ONTAP ; cependant, ce processus est facultatif.
...
[NOTE]
====
* À partir du 15 juin 2025, vous ne pourrez plus installer ni mettre à niveau ONTAP Mediator 1.8 et 1.9, car les certificats de vérification de signature de code ont expiré. Installez ou mettez à niveau ONTAP Mediator 1.9.1.
...
`ontap-mediator-1.9.1`
 a| 
Exécutable d'installation du produit pour la version 1.9.1
...
----
. Vérifiez les `ontap-mediator-1.9.1.sig.tsr` fichiers et `ontap-mediator-1.9.1.tsr` à l'aide des certificats associés :
+
`openssl ts -verify`
