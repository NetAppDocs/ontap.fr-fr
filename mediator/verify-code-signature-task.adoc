...
[NOTE]
====
* À partir du 15 juin 2025, vous ne pourrez plus installer ni mettre à niveau ONTAP Mediator 1.8 et 1.9, car les certificats de vérification de signature de code ont expiré. Installez ou mettez à niveau ONTAP Mediator 1.10.
...
`ontap-mediator-1.10`
 a| 
L'exécutable d'installation du produit pour la version 1.10
...
----
. Vérifiez le  `ontap-mediator-1.10.sig.tsr` et  `ontap-mediator-1.10.tsr` fichiers utilisant les certificats associés :
+
`openssl ts -verify`
