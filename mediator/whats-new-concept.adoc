...
---
= Nouvelles fonctionnalités d' ONTAP Mediator
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
** Compatible : 10.1 et 9.7.
** Recommandé : 10.2, 10.0, 9.8, 9.6, 9.4 et 8.10.


* Prise en charge de Rocky Linux 10.2, 9.8 et 8.10.
* Prise en charge d'Oracle Linux 10.1 et 9.8 avec les noyaux rhck et uek.
* À partir d'ONTAP Mediator 1.12.1 et ONTAP 9.19.1, une seule instance d'ONTAP Mediator configurée avec HTTPS peut surveiller simultanément les configurations suivantes :
+
** Un maximum de dix configurations IP MetroCluster à quatre nœuds ou cinq à huit nœuds si elles sont installées sur un hôte doté d'un processeur à 8 cœurs et d'au moins 16 Go de RAM. Voir link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/concept_mediator_requirements.html#mediator-multiple-mccip["Utilisez ONTAP Mediator pour surveiller plusieurs configurations IP MetroCluster"^].
** Une configuration IP MetroCluster à quatre nœuds et une relation de synchronisation active entre pairs SnapMirror si elle est installée sur un hôte doté d’un processeur à 2 cœurs. Voir link:../snapmirror-active-sync/mediator-install-task.html#ontap-mediator["Configurer le médiateur ONTAP pour la synchronisation active de SnapMirror"].


* Inclut un nouvel outil pour réparer une installation SCST défectueuse. Après l'installation d'ONTAP Mediator, vous pouvez trouver l'outil à  `/usr/local/bin/mediator_repair_scst`. Vous pouvez utiliser l'outil après une mise à niveau du système d'exploitation, si le système d'exploitation mis à niveau requiert un correctif SCST qui n'est pas disponible dans le package SCST par défaut.
...
* Prise en charge de Rocky Linux 10.2, 9.8 et 8.10.
* Prise en charge d'Oracle Linux 10.1 et 9.7 avec les noyaux rhck et uek.
* Prise en charge de la configuration d'un médiateur ONTAP qui utilise HTTPS pour la communication dans les configurations IP MetroCluster. HTTPS offre une sécurité accrue en chiffrant l'intégralité des sessions à l'aide de SSL/TLS. Vous pouvez choisir d'installer le médiateur ONTAP qui utilise uniquement le protocole HTTPS. Le package SCST n'est pas installé si cette méthode d'installation est utilisée. link:index.html#https-iscsi-installation["En savoir plus sur les options d'installation HTTPS et iSCSI pour ONTAP Mediator"].
...
1,11
 a| 
* Prise en charge de RHEL 10.1, 10.0, 9.7, 9.6, 9.4 et 8.10.
* Prise en charge de Rocky Linux 10.1, 9.7 et 8.10.
* Prise en charge d'Oracle Linux 10.0 et 9.6 avec les noyaux rhck et uek.
* Ajoute la prise en charge d'IPv6 pour les configurations IP de MetroCluster .
* Ajoute la prise en charge de fapolicyd.
...
1,10
 a| 
* Prise en charge de RHEL 10.0, 9.6, 9.4 et 8.10.
* Prise en charge de Rocky Linux 10.0, 9.6 et 8.10.
* Met à jour la version Python de base de Python 3.9 à Python 3.12.
...
* Ajoute de nouveaux certificats pour la vérification de la signature du code.
...
* Le programme d'installation affiche des avertissements lorsqu'il détecte des certificats de signature de code expirés.
...
* Une signature cryptographique a été ajoutée au programme d'installation.
...
| Système d'exploitation pour le médiateur ONTAP | 1.12.1 | 1,12 | 1,11 | 1,10 | 1.9.1 | 1,9 | 1.8 | 1.7 | 1.6 | 1.5 | 1.4 | 1.3 | 1.2 | 1.1 | 1.0 
...
| Médiateur ONTAP 1.11 | scst-3.9.tar.gz 
