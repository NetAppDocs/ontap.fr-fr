...
* À partir d'ONTAP 9.19.1 et ONTAP Mediator 1.12, vous pouvez configurer un ONTAP Mediator qui utilise HTTPS pour la communication dans les configurations IP MetroCluster. HTTPS offre une sécurité accrue en chiffrant l'intégralité des sessions à l'aide de SSL/TLS. Vous pouvez choisir d'installer ONTAP Mediator avec la prise en charge de HTTPS uniquement. Le package SCST n'est pas installé si cette méthode d'installation est utilisée. link:index.html#https-iscsi-installation["En savoir plus sur les options d'installation HTTPS et iSCSI pour ONTAP Mediator"].
