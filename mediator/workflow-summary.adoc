...
.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-2.png["Deux"]link:upgrade-host-os-mediator-task.html["Mise à niveau du système d'exploitation hôte et du Mediator"]
[role="quick-margin-para"]
Si vous mettez à niveau une version existante d'ONTAP Mediator, vous devez d'abord désinstaller la version précédente, puis installer la nouvelle version. Si vous installez ONTAP Mediator pour la première fois, vous pouvez ignorer cette étape.
