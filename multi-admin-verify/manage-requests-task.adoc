...
Lorsqu'une nouvelle demande d'administration est créée, le système vérifie une demande existante pour suppression en fonction des critères suivants :

* La demande doit être expirée ou exécutée.
* Si la demande a plus d'un jour après l'expiration de son délai d'approbation, elle est supprimée.
* S'il y a plus de 1 000 requêtes dans le tableau, une requête plus ancienne est supprimée pour maintenir la limite.


Si le nombre de demandes dépasse 1000 après ce nettoyage, un avertissement est émis aux administrateurs.
