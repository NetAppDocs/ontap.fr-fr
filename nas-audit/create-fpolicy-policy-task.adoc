...
+
** Vous pouvez ajouter un ou plusieurs événements séparés par des virgules à la politique FPolicy.
...
+
La commande suivante crée une stratégie nommée `policy1` qui a l'événement nommé `event1` et le moteur externe nommé `engine1` associés. Cette stratégie utilise les valeurs par défaut dans la configuration de la stratégie :
`vserver fpolicy policy create -vserver vs1.example.com -policy-name policy1 -events event1 -engine engine1`

+
La commande suivante crée une stratégie nommée `policy2` qui a l'événement nommé `event2` et le moteur externe nommé `engine2` associés. Cette stratégie est configurée pour utiliser un accès privilégié avec le nom d'utilisateur spécifié. L'accès en lecture directe est activé :
...
+
La commande suivante crée une stratégie nommée `native1` qui a l'événement nommé `event3` associé. Cette stratégie utilise le moteur natif et utilise les valeurs par défaut dans la configuration de la stratégie :
