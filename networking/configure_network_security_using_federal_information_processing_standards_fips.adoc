...
summary: Conforme à la norme Federal Information Processing Standards 140-2 pour toutes les connexions SSL. 
---
= Configurer la sécurité réseau ONTAP à l'aide de FIPS
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
|===
Vous devez reconfigurer les comptes de clés publiques SSH existants pour utiliser un type de clé pris en charge avant d'activer FIPS, sinon l'authentification de l'administrateur échouera.
...
. À partir d'ONTAP 9.9.1, le redémarrage n'est pas nécessaire. Si vous utilisez ONTAP 9.8 ou une version antérieure, redémarrez manuellement chaque nœud du cluster individuellement.
...
Vous pouvez activer SSLv3 avec la commande suivante. Si vous utilisez ONTAP 9.9.1 ou une version ultérieure, vous ne voyez pas le message d'avertissement.
...
.Étapes
. Si vous utilisez ONTAP 9.8 ou une version antérieure, redémarrez manuellement chaque nœud du cluster individuellement.
...
* link:../networking/configure-tls-offload.html["Configurer le déchargement matériel TLS"]
