...
|===
Le `sfo-partner-only` La règle de basculement n'est pas une valeur par défaut, mais peut être utilisée lorsque vous souhaitez que la LIF bascule vers un port du nœud de rattachement ou du partenaire SFO uniquement.
