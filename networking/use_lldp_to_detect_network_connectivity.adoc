...
Le protocole LLDP doit également être activé sur tous les commutateurs et routeurs Ethernet avant que les informations relatives aux périphériques voisins puissent être affichées.
...
* Les informations affichées peuvent différer entre le cluster et le commutateur Ethernet.
+
Les données d'identification du châssis et du port affichées par le commutateur Ethernet peuvent être différentes pour les ports CNA et non-CNA.
