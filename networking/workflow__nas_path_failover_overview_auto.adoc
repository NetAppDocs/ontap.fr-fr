...
summary: Configuration de basculement du chemin NAS pour ONTAP 9.8 et versions ultérieures. 
---
= Workflow de basculement de chemin NAS pour ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: Une interface LIF SAN ne migre pas (sauf si vous la déplacez manuellement après une panne de liaison). En revanche, la technologie de routage multiple de l’hôte redirige le trafic vers une autre interface LIF. Pour plus d’informations, consultez link:../san-admin/index.html["Administration SAN"].
