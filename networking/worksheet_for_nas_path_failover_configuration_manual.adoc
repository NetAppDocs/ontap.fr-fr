...
---
= Fiche technique de configuration de basculement de chemin NAS pour ONTAP 9.0 - 9.7
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* Liste des adresses IP pour les serveurs DNS qui fourniront la résolution de nom pour le serveur NFS ou SMB.
* Les serveurs DNS répertoriés doivent contenir les enregistrements SRV (Service Location Records) nécessaires pour localiser les serveurs LDAP Active Directory et les contrôleurs de domaine du domaine auquel le serveur SMB sera rattaché.
