...
====
* Les clients doivent prendre en charge RPC avec TLS sur TCP, être configurés pour demander TLS lors du montage et faire confiance à l'autorité de certification ayant signé le certificat du serveur NFS. Les étapes de configuration côté client ne sont pas documentées ici car elles varient selon le client. Pour une procédure de bout en bout pour un client Linux, consultez link:https://docs.netapp.com/us-en/ontap-technical-reports/nfs-tls/nfs-tls-configure.html["Configurer NFS via TLS"].
