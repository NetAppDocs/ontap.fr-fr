...
summary: 'À partir d"ONTAP 9.5, la prise en charge des noms encodés en UTF-8 sur 4 octets permet la création et l"affichage de noms de fichiers, de répertoires et d"arborescences incluant des caractères supplémentaires Unicode en dehors du Basic Multilingual Plane. Dans les versions précédentes, ces caractères supplémentaires ne s"affichaient pas correctement dans les environnements multiprotocoles.' 
---
= En savoir plus sur la gestion ONTAP NFS des noms de fichiers, de répertoires et de qtree multi-octets
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
