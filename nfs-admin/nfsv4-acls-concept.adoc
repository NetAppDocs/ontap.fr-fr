...
summary: 'Un client utilisant les listes de contrôle d"accès (ACL) NFSv4 peut définir et consulter les ACL des fichiers et répertoires du système. Lorsqu"un nouveau fichier ou sous-répertoire est créé dans un répertoire doté d"une ACL, le nouveau fichier ou sous-répertoire hérite de toutes les entrées de contrôle d"accès de cette ACL qui ont été marquées avec les indicateurs d"héritage appropriés.' 
---
