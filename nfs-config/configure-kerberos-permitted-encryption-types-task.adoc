...
summary: 'Par défaut, ONTAP prend en charge les types de chiffrement suivants pour NFS Kerberos : DES, 3DES, AES-128 et AES-256. Vous pouvez configurer les types de chiffrement autorisés pour chaque SVM afin de répondre aux exigences de sécurité de votre environnement particulier à l’aide de la commande vserver nfs modify avec le paramètre -permitted-enc-types.' 
---
