...
[NOTE]
====
Le `-servers` Cette option est obsolète dans ONTAP 9.2. À partir de ONTAP 9.2, le `-ldap-servers` remplace le `-servers` légale. Ce nouveau champ peut prendre un nom d'hôte ou une adresse IP pour le serveur LDAP.
