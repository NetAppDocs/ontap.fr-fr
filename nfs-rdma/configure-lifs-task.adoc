...
NOTE: Chaque port RDMA peut avoir un maximum de 127 LIF. Lorsque vous ajoutez davantage de LIF, ou lorsqu'une LIF bascule vers un port qui dépasse cette limite, les LIF nouvellement ajoutées ou la LIF basculée ne peuvent pas servir le trafic RDMA.
...
. Sélectionnez la machine virtuelle de stockage et le nœud hôte. Attribuez un *Nom*, une *Adresse IP* et un *Masque de sous-réseau*.
...
** Le  `-rdma-protocols`paramètre accepte une liste, qui est vide par défaut. Lorsque  `roce`est ajouté comme valeur, la LIF ne peut être configurée que sur des ports prenant en charge le déchargement RoCE, ce qui affecte à la fois la migration et le basculement de la LIF.
