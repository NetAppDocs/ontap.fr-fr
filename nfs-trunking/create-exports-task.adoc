...
Exigences d'export ONTAP :

* La règle d'export par défaut du volume racine SVM doit inclure une règle autorisant tous les clients à accéder librement via NFS.
+
Apprenez-en davantage sur link:../nfs-config/open-export-policy-svm-root-volume-task.html["Accès client NFS sur la SVM ONTAP."]

* Vous devez ajouter la même règle à la politique par défaut si vous avez créé une règles d'export personnalisée.
+
Apprenez-en davantage surlink:../nfs-admin/default-export-policy-svms-concept.html["Règles d'export par défaut pour les SVM NFS ONTAP."]
