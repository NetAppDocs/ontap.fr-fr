...
[role="lead"]
Avant de configurer le peering de clusters, vous devez vérifier que les exigences en matière de connectivité, de port, d'adresse IP, de sous-réseau, de pare-feu et de cluster sont respectées.
...
== Limites entre cluster

Le nombre d'entre cluster pris en charge dépend de la version d'ONTAP :

* ONTAP 9.14.1 et versions ultérieures : jusqu’à 400 entre cluster sont pris en charge.
* Versions ONTAP antérieures : jusqu’à 255 entre cluster sont pris en charge.
+
....
[NOTE]
