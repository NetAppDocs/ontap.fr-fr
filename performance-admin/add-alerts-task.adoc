...
sidebar: sidebar 
keywords: add, alerts 
summary: 'Les événements sont des notifications générées automatiquement par Active IQ Unified Manager lorsqu"une condition prédéfinie se produit ou lorsqu"une valeur de compteur de performance franchit un seuil. Les événements vous aident à identifier les problèmes de performance dans les clusters que vous surveillez.' 
---
= Ajouter des alertes ONTAP Active IQ Unified Manager
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
