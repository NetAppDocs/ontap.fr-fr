...
| link:../snaplock/index.html#multi-admin-verification-mav-support.md["Vérification multi-administrateurs"]  a| 
...
| NDMP et extension de groupe de cohérence sans interruption avec SnapMirror Business Continuity | C'est possible link:../smbc/smbc_admin_add_and_remove_volumes_in_consistency_groups.html["Ajoutez des volumes à un groupe de cohérence sans interruption avec une relation SM-BC active"] et link:../smbc/supported-configurations-reference.html#ndmp-restore["Utilisez la restauration NDMP avec SM-BC"]. 


| link:link:../consistency-groups/protect-task.html#configure-asynchronous-snapmirror-protection["SnapMirror asynchrone avec des groupes de cohérence uniques"] | Les groupes de cohérence prennent en charge les configurations SnapMirror asynchrones, ce qui permet l'archivage des sauvegardes SnapMirror pour les groupes de cohérence uniques. 
|===
...
| IPv6 lors de la configuration de clusters sur les plateformes A800 et FAS8700 | Sur les plateformes A800 et FAS8700, vous pouvez utiliser l'interface de ligne de commandes ONTAP pour créer et configurer de nouveaux clusters dans des environnements réseau IPv6 uniquement. 
...
| xref:..san-admin/create-nvme-namespace-subsystem-task.html[NVMe/FC sur des hôtes AIX] | ONTAP prend en charge le protocole NVMe/FC sur des hôtes AIX. Voir la link:https://mysupport.netapp.com/matrix/["Outil d'interopérabilité NetApp"^] pour les configurations prises en charge. 
|===
...
* Fonctionnalité de vérification multiadministrateur avec la protection anti-ransomware autonome
...
| xref:../encryption-at-rest/manage-external-key-managers-sm-task.html[Prise en charge de la gestion du gestionnaire de clés externe] | System Manager vous permet de gérer des gestionnaires de clés externes afin de stocker et de gérer les clés d'authentification et de chiffrement. 
|===