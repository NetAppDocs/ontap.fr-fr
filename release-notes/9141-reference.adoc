...
| xref:../anti-ransomware/manage-parameters-task.html[Alertes réglables pour la protection anti-ransomware autonome] | Configurez la protection anti-ransomware autonome (ARP) pour recevoir des notifications lorsqu'une nouvelle extension de fichier est détectée ou lorsqu'une copie Snapshot ARP est prise, et recevez un avertissement précoce en cas d'événements de ransomware. 
