...
| Protection autonome contre les ransomwares (ARP/AI) | ARP/AI est automatiquement activé par défaut sur tous les nouveaux volumes après un délai de grâce de 12 heures sur les plateformes prises en charge. | ONTAP 9.18.1 
...
| Durée de vie (TTL) des informations d'identification positives mises en cache pour les SVM ONTAP | À partir d'ONTAP 9.17.1, la durée de vie (TTL) des informations d'identification positives mises en cache pour les SVM ONTAP a diminué de 24 heures (86 400 000 millisecondes) à 1 heure (3 600 000 millisecondes. | ONTAP 9.17.1 
...
| Cloud SnapMirror | SnapMirror cloud augmente le nombre de compartiments S3 pris en charge de 6 à 100. | ONTAP 9.19.1 


| Limite de contrôle de flux de connexion S3 | La limite de contrôle de flux pour les connexions ONTAP S3 a été réduite de 2 Mo à 256 Ko pour les instances cloud disposant de ≤ 32 Go de mémoire. | ONTAP 9.19.1 


| Délai d'inactivité de la connexion S3 | Le délai d'inactivité de la connexion S3 a été réduit de 20 minutes à 5 minutes. | ONTAP 9.19.1 
...
| Entre cluster | Limite entre cluster prise en charge augmentée de 255 à 400, améliorant l'évolutivité pour les environnements comportant un grand nombre de relations entre cluster. | ONTAP 9.14.1 
...
* Le nombre maximal de constituants dans un FlexGroup ne peut pas dépasser 24.
* Les limites de volume SVM-DR sont de 500 par nœud, 1000 par paire haute disponibilité (y compris les volumes FlexVol et les constituants FlexGroup).
+

NOTE: Les limites sont spécifiques à chaque plateforme. Vous devez vérifier les limites appropriées pour votre configuration en accédant à link:https://hwu.netapp.com["NetApp Hardware Universe"^] et en sélectionnant votre plateforme et la version d'ONTAP.
...
| Transfert simultané SnapMirror | Le nombre maximal de transferts simultanés asynchrones au niveau du volume est passé de 100 à 200.  Le nombre de transferts SnapMirror Cloud est passé de 32 à 100 sur les systèmes haut de gamme et de 6 à 20 sur les systèmes bas de gamme. | ONTAP 9.8 
