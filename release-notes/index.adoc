...
* Apprenez-en plus sur https://docs.netapp.com/us-en/ontap-afx/release-notes/whats-new-9191.html["nouvelles fonctionnalités logicielles ONTAP améliorées pour les systèmes de stockage NetApp AFX"^].
* Apprenez-en plus sur https://docs.netapp.com/us-en/asa-r2/release-notes/whats-new-9191.html["nouvelles fonctionnalités logicielles ONTAP améliorées pour les systèmes de stockage NetApp ASA r2"^].
...
== Points saillants d'ONTAP 9.19.1

ONTAP 9.19.1 offre des fonctionnalités nouvelles et améliorées en matière de sécurité, d'évolutivité et de gestion du stockage afin de soutenir la modernisation de l'infrastructure de données. Ces améliorations vous aident à évoluer, à optimiser vos opérations et à sécuriser les données pour l'avenir. Pour obtenir la liste complète des nouvelles fonctionnalités et améliorations, consultez xref:whats-new-9191.adoc[Nouveautés dans ONTAP 9.19.1].

* xref:../performance-admin/guarantee-throughput-qos-task.html[Politiques de rafale QoS configurables]
+
Les politiques de rafales QoS configurables permettent des pics de performance contrôlés et limités dans le temps au-delà des limites QoS, améliorant ainsi la réactivité de la charge de travail lors des pics d'activité sans sacrifier l'équité ni nécessiter de surdimensionnement.

* xref:../nfs-admin/enable-nfs-over-tls-task.html[NFS sur TLS 1.3]
+
Sécurise les données NFS en transit grâce au chiffrement TLS 1.3 basé sur les standards, contribuant ainsi à renforcer la protection et à simplifier l'adoption du NFS chiffré à l'échelle de l'entreprise. 

* xref:../s3-config/regenerate-access-keys.html[Plusieurs clés d'accès S3 par utilisateur]
+
Ajoute la prise en charge de plusieurs clés d'accès S3 actives par utilisateur, permettant une rotation transparente des identifiants sans indisponibilité des applications et aidant les clients à renforcer la sécurité et la conformité tout en maintenant un accès ininterrompu aux charges de travail d'objets.

* xref:../snapmirror-active-sync/protect-nas-task.html[SnapMirror active sync pour NAS]
+
Grâce à la prise en charge NAS, SnapMirror active sync est désormais une solution de continuité d'activité unifiée offrant un RPO nul avec un basculement transparent. 

* xref:../san-config/configure-fc-nvme-hosts-ha-pairs-reference.html[FC à connexion directe]
+
Introduit la prise en charge sécurisée des cartes HBA FC, permettant une connectivité hôte fiable et établissant une base pour une sécurité FC en vol reposant sur le matériel, tout en préservant les performances et en répondant aux exigences de conformité des applications d'entreprise. 

* xref:../san-admin/multipath-automatic-lif-failover-support.html[Basculement LIF actif-actif unifié]
+
Les systèmes AFF offrent un multipathing actif-actif avec basculement automatique LIF, maintenant tous les chemins SAN actifs et optimisés afin que les E/S de l'hôte continuent immédiatement pendant le basculement. 

* link:https://docs.netapp.com/us-en/ontap-fli/get-started/learn-about-data-migration.html["Prise en charge iSCSI dans FLI"^]
+
Ajoute la prise en charge iSCSI pour l'importation de LUN étrangères (FLI), permettant de migrer des données de blocs depuis des systèmes de stockage tiers vers ONTAP en utilisant la connectivité iSCSI existante avec une interruption minimale. 

* xref:../s3-config/create-modify-bucket-policy-task.html[Écritures et suppressions conditionnelles S3]
+
Permet la prise en charge des écritures et suppressions conditionnelles avec l'API S3, permettant aux applications d'appliquer des contrôles de cohérence des données lors des mises à jour et des suppressions d'objets, réduisant ainsi le risque d'écrasements ou de suppressions accidentels dans les charges de travail d'objets simultanées et distribuées.

* xref:../concepts/path-failover-concept.html#nas-path-failover[Basculement LIF combiné]
+
...
* xref:../networking/improve-tcp-wan-performance.html[Performances WAN améliorées]
+
Les performances WAN améliorées grâce à TCP RACK optimisent le débit et la résilience sur les réseaux congestionnés, accélérant l'accès aux fichiers à distance et les services de données intersites. 

* xref:../networking/configure-tls-offload.html[kTLS accéléré par matériel pour un chiffrement plus rapide du cluster principal]
+
Améliore le chiffrement du cluster principal en introduisant le kTLS accéléré par le matériel, déchargeant le traitement du chiffrement sur les SmartNIC compatibles afin d’offrir des performances supérieures et une surcharge CPU réduite, tout en maintenant une communication sécurisée et permanente sur le trafic du cluster et de la haute disponibilité (HA). 

* xref:../anti-ransomware/use-cases-restrictions-concept.html#snapmirror[Détection de ransomware par IA pour les charges de travail répliquées de manière synchrone]
+
Étend la protection autonome contre les ransomwares (ARP/AI) à SnapMirror synchrone pour les environnements SAN et NAS, ainsi qu'à SnapMirror active sync SAN, en préservant les signaux de détection et de récupération dans les architectures à RPO zéro. 
...
+
Fournit une interface graphique conviviale pour la gestion des SVM, éliminant ainsi le besoin d'une interface de ligne de commande complexe. 
...
== Points saillants ONTAP 9.18.1

ONTAP 9.18.1 offre des fonctionnalités nouvelles et améliorées dans les domaines de la sécurité, de l'évolutivité et de la gestion du stockage pour soutenir la modernisation de l'infrastructure de données.  Ces améliorations vous permettent d'évoluer, d'opérer plus efficacement et de sécuriser vos données pour l'avenir.  Pour obtenir la liste complète des nouvelles fonctionnalités et améliorations, consultezxref:whats-new-9181.adoc[Nouveautés d' ONTAP 9.18.1] .

* xref:../networking/configure-backend-encryption.html[Chiffrement HA du cluster backend]
+
Possibilité d'utiliser mTLS pour chiffrer le réseau du cluster backend.

* xref:../networking/configure_network_security_using_federal_information_processing_standards_fips.html[Prise en charge PQC pour les données en transit]
+
Capacité à utiliser des algorithmes post-quantiques pour les données en transit.

* xref:../networking/ipsec-prepare.html[Prise en charge d'IPSec IPv6 avec déchargement matériel]
+
Prise en charge d'IPSec avec déchargement matériel et adresse IPv6.

* xref:../anti-ransomware/use-cases-restrictions-concept.html#supported-configurations[Volumes FlexGroup de support ARP/AI]
+
Les volumes NetApp FlexGroup prennent en charge ARP/AI, offrant une détection et une protection avancées contre les ransomwares basées sur l'apprentissage automatique.

* xref:../san-admin/nvme-copy-offload.html[Déchargement de copie pour NVMe]
+
Accélère les transferts de données volumineux en déchargeant directement les opérations de copie sur le stockage, réduisant ainsi la charge du processeur hôte et du réseau.

* xref:../s3-snapshots/list-restore-s3-snapshot.html[Restauration d'instantanés de compartiment S3 pilotée par le stockage]
+
...
ONTAP 9.17.1 offre des innovations en matière de protection autonome contre les ransomwares basée sur l'IA pour SAN et ajoute la prise en charge de l'hôte frontal NVMe avec la synchronisation active SnapMirror et la gestion hiérarchique des groupes de cohérence.  Ces améliorations renforcent le leadership de NetApp en matière de sécurité des données et de cyber-résilience, améliorant les performances, l'évolutivité, l'efficacité et la continuité des activités ONTAP . Pour une liste complète des nouvelles fonctionnalités et améliorations, consultez la page xref:whats-new-9171.adoc[Quoi de neuf dans ONTAP 9.17.1] .

* xref:../anti-ransomware/index.html[Prise en charge du SAN avec protection autonome contre les ransomwares]
+
ARP/AI prend en charge les volumes SAN avec détection d'anomalies basée sur le chiffrement, introduit de nouvelles commandes pour des statistiques d'entropie détaillées et unifie la messagerie de protection contre les ransomwares dans System Manager, auparavant axée sur le NAS. Des seuils de détection configurables et une conservation des snapshots plus déterministe offrent une plus grande flexibilité pour diverses charges de travail.

* xref:../system-admin/use-hsts-task.html[Prise en charge de HTTP Strict Transport Security (HSTS)]
+
ONTAP 9.17.1 prend en charge HTTP Strict Transport Security pour les services Web, permettant l'application d'une communication HTTPS sécurisée entre le navigateur d'un utilisateur et ONTAP.

* xref:../networking/ipsec-prepare.html[Déchargement matériel IPsec avec groupes d'agrégation de liens]
+
ONTAP 9.17.1 prend en charge le déchargement matériel IPsec pour les groupes d'agrégation de liens, étendant la prise en charge du déchargement matériel introduite dans ONTAP 9.16.1.
...
+
SnapMirror Active Sync prend désormais en charge l'accès NVMe pour les charges de travail VMware avec accès hôte NVMe/TCP et NVMe/FC pour les clusters ONTAP à deux nœuds.
...
+
ONTAP 9.16.1 ajoute davantage de commandes au framework MAV pour une protection supplémentaire contre les menaces internes malveillantes. Ces améliorations incluent de nombreuses commandes de gestion des groupes de cohérence (CG), de VScan, de la protection autonome contre les ransomwares (ARP/AI), ainsi que des commandes de configuration NVMe.

* xref:../anti-ransomware/index.html[Protection anti-ransomware autonome avec améliorations basées sur l'IA (ARP/ai)]
+
La protection autonome contre les ransomwares a été mise à niveau grâce à de nouvelles fonctionnalités d'IA, lui permettant de détecter et de contrer les attaques de ransomware avec une précision et un rappel de 99 %. Comme l'IA est entraînée sur un ensemble de données exhaustif, il n'y a plus de période d'apprentissage pour ARP/AI fonctionnant sur des volumes FlexVol et ARP/AI démarre immédiatement en mode actif. ARP/AI introduit également une fonctionnalité de mise à jour automatique indépendante d'une mise à niveau d'ONTAP afin de garantir une protection et une résilience constantes contre les menaces les plus récentes.
...
+
La désallocation d'espace (également appelée « perforation » et « unmap ») est désormais prise en charge pour les espaces de noms NVMe.  La désallocation d’espace permet aux volumes à provisionnement dynamique et aux espaces de noms NVMe de récupérer l’espace inutilisé lorsque les données sont supprimées par l’application hôte.  Cela améliore considérablement l’efficacité globale du stockage, en particulier avec les systèmes de fichiers qui ont un taux de rotation des données élevé.
...
+
À partir d'ONTAP 9.15.1, vous avez également la possibilité de sauvegarder le lien symbolique lui-même plutôt que les données auxquelles il fait référence. Cela peut offrir plusieurs avantages, notamment une amélioration des performances de vos applications de sauvegarde. Vous pouvez activer cette fonctionnalité à l'aide de l'interface de ligne de commande ONTAP ou de l'API REST ONTAP.
...
** xref:../anti-ransomware/support-arpai.html#multi-admin-verification-with-arp-ai-protected-volumes[Prise en charge de la vérification multiadministrateur]
+
...
+
Si les constituants FlexGroup deviennent déséquilibrés, FlexGroup peut être rééquilibré et géré sans interruption depuis l’interface de ligne de commande ONTAP, l’API REST ONTAP et System Manager. Pour des performances optimales, les membres constituants d’un FlexGroup doivent avoir leur capacité utilisée répartie de manière homogène.
...
* Améliorations apportées à System Manager
+
System Manager télécharge automatiquement les mises à jour du micrologiciel pour les disques, les étagères et les processeurs de service, en plus de fournir de nouvelles intégrations avec Active IQ Digital Advisor (également connu sous le nom de Digital Advisor), NetApp Console et la gestion des certificats.  Ces améliorations simplifient l’administration et maintiennent la continuité des activités.
