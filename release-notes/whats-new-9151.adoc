...
xref:../smb-admin/windows-backup-symlinks.html[Applications de sauvegarde Windows et liens symboliques de style Unix sur les serveurs]
 a| 
Lorsqu'une application de sauvegarde Windows rencontre un lien symbolique (symlink) de type Unix, le lien est suivi et les données réelles sont renvoyées par ONTAP et sauvegardées. À partir d'ONTAP 9.15.1, vous avez également la possibilité de sauvegarder le lien symbolique lui-même plutôt que les données vers lesquelles il pointe. Cela peut offrir plusieurs avantages, notamment une amélioration des performances de vos applications de sauvegarde. Vous pouvez activer cette fonctionnalité à l'aide de l'interface de ligne de commande ONTAP (CLI) ou de l'API REST ONTAP.
...
Les groupes de cohérence qui utilisent la protection asynchrone SnapMirror prennent désormais en charge jusqu'à 80 volumes dans le groupe de cohérence.
 ...
 a| 
xref:../consistency-groups/configure-task.html[Prise en charge du niveau de droit d'administration pour les opérations de l'API REST ONTAP et de l'interface de ligne de commande ONTAP avec les groupes de cohérence]
 a| 
Les opérations ONTAP CLI et ONTAP REST API pour les groupes de cohérence sont désormais prises en charge au niveau du droit d'administration.
...
xref:../concepts/ontap-and-vmware.html[Réserves persistantes pour les volumes virtuels VMware avec le clustering avec basculement sur incident Windows Server]
 a| 
ONTAP prend actuellement en charge les volumes virtuels VMware (vVols) ainsi que les réservations persistantes avec les LUN traditionnels. À partir d'ONTAP 9.15.1, vous pouvez également créer une réservation persistante avec un vVol. La prise en charge de cette fonctionnalité est implémentée dans ONTAP tools for VMware vSphere 9. Elle est uniquement prise en charge dans un cluster de basculement Windows Server (WSFC), qui est un groupe de machines virtuelles Windows en cluster.
