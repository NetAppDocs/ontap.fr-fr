...
| Mise à jour | Description 
 ...
 a| 
xref:../s3-multiprotocol/index.html[Prise en charge de l'action ObjectCopy dans les compartiments NAS ONTAP S3]
 a| 
L'action ObjectCopy est prise en charge dans le bucket NAS ONTAP S3.



 a| 
xref:../s3-multiprotocol/index.html#object-multipart-upload[Prise en charge de la liaison d'un bucket NAS S3 à un chemin de jonction]
 a| 
...
xref:../s3-multiprotocol/index.html#object-multipart-upload[Prise en charge multiprotocole S3 pour le balisage et les métadonnées]
 a| 
Les paires clé/valeur de marquage et de métadonnées utilisateur sont prises en charge par l'action CreateMultipartUpload avec le multiprotocole S3.
