...
* link:arp-warning-clear-revert.html["Répondez aux alertes de protection anti-ransomware autonome en cas d'activité anormale"].




| ONTAP 9.19.1  a| 
* Si vous avez des SVM avec des configurations de chemins d'accès multiples actifs-actifs, vous devez link:delete-active-active-svms.html["supprimer les SVM"].
* Si vous avez modifié les paramètres par défaut d'un groupe de stratégie QoS créé dans ONTAP 9.19.1 ou version ultérieure, vous devez link:modify-qos-policy-groups.html["réinitialisez-les"].
* Si vous avez mis à niveau des volumes vers ONTAP 9.19.1 et ajouté une deuxième clé d'accès S3 à un utilisateur, vous devez link:second-s3-access-key.html["retirez ces clés"].




| ONTAP 9.18.1  a| 
* Si l'activation automatique a été configurée pour ARP/AI dans le cadre d'une mise à niveau ONTAP 9.18.1, vous devrez link:anti-ransomware-disable-auto-enable.html["désactivez-le"].




| ONTAP 9.17.1  a| 
* Si vous avez activé la fonctionnalité ONTAP ARP/AI pour SAN, link:anti-ransomware-disable-san.html["désactivez-le"].
