...
Vous travaillez avec le  `ext_performance_monitoring.enabled` paramètre en utilisant le  `/api/storage/qtrees` point de terminaison. Utilisez GET pour récupérer la valeur actuelle, POST pour la définir lors de la création d'un nouveau qtree, et PATCH pour la modifier sur un qtree existant.
