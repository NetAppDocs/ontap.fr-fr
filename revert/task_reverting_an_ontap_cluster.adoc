...
. Après que les deux nœuds ont été rétablis, réactivez la haute disponibilité (HA) pour le cluster :
+
[source, cli]
----
...
NOTE: La réactivation de la haute disponibilité du cluster peut entraîner que le paramètre  `storage failover auto-giveback` soit rétabli sur true.
...
----
. Vérifiez le  `storage failover auto-giveback` paramètre sur les deux nœuds :
+
[source, cli]
----
...
----
+
Si une configuration différente est requise pour votre environnement, utilisez la commande  `storage failover modify -node <nodename> -auto-giveback <true|false>` pour la modifier.
...
* link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/Why_auto-giveback_enabled_after_re-enabling_storage_failover_setting["NetApp Base de connaissances : Pourquoi la restitution automatique est-elle activée après la réactivation du paramètre de basculement de stockage ?"^]
