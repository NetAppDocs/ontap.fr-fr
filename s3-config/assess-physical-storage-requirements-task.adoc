...
Lorsque vous créez un compartiment S3 dans une SVM compatible S3, un volume FlexGroup est link:../s3-config/architecture.html#automatic-flexgroup-sizing-with-ontap-9-14-1-and-later["créé automatiquement"] créé pour prendre en charge le compartiment. Vous pouvez laisser ONTAP sélectionner automatiquement les agrégats et composants FlexGroup sous-jacents (par défaut), ou vous pouvez sélectionner vous-même les agrégats et composants FlexGroup sous-jacents.
...
* https://www.netapp.com/pdf.html?item=/media/17251-tr4571a.pdf["Rapport technique NetApp 4571-a : meilleures pratiques relatives au volume NetApp ONTAP FlexGroup"^]
