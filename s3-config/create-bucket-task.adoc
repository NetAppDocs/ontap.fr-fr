...
*** Si vous ne spécifiez pas d’agrégats et de constituants pour un nouveau compartiment, le nouveau compartiment sera ajouté à un FlexGroup existant, mais uniquement si le paramètre  `retention-mode` correspond au paramètre  `retention-mode` des compartiments existants et au type FlexGroup (`compliance`, `non-snaplock`, `enterprise`. Voir xref:../flexgroup/index.html[Gestion des volumes FlexGroup] pour plus d’informations.
+
