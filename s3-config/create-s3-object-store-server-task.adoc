...
Lors de la création d'un serveur de stockage d'objets, un utilisateur racine avec l'UID 0 est créé. Aucune clé d'accès ni clé secrète n'est générée pour cet utilisateur racine. L'administrateur ONTAP doit exécuter la commande  `object-store-server user regenerate-keys` pour définir la clé d'accès et la clé secrète pour cet utilisateur.
