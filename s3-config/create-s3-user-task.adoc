...
Pour maintenir l'accès aux ressources S3 si la clé d'accès d'origine expire, à partir d'ONTAP 9.19.1, vous pouvez link:regenerate-access-keys.html#generate-a-second-s3-access-key["générer une deuxième clé d'accès"] pour chaque utilisateur S3.
...
.System Manager
--
. Sélectionnez *Stockage > Machines virtuelles de stockage*. Sélectionnez la machine virtuelle de stockage à laquelle vous souhaitez ajouter un utilisateur, sélectionnez *Paramètres* puis sélectionnez image:icon_pencil.gif["Icône Modifier"] sous S3.
. Pour ajouter un utilisateur, sélectionnez *Utilisateurs > Ajouter*.
...
. Sélectionnez *Enregistrer*. L'utilisateur est créé et une clé d'accès ainsi qu'une clé secrète sont générées pour lui.
