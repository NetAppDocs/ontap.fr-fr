...
. Un client LDAP est configuré avec TLS activé sur la VM de stockage. Voir link:../nfs-config/create-ldap-client-config-task.html["Créez une configuration client LDAP"] et .
. Associer la configuration client au Vserver. Voir link:../nfs-config/enable-ldap-svms-task.html["Associer la configuration client LDAP aux SVM"] et https://docs.netapp.com/us-en/ontap-cli//vserver-services-name-service-ldap-create.html["création du ldap nom-service des services vserver"].
. Si vous utilisez une VM de stockage de données, créez une interface réseau de gestion (LIF) et sur la VM, ainsi qu'une politique de service pour la LIF. Voir la https://docs.netapp.com/us-en/ontap-cli/network-interface-create.html["création d'interface réseau"] et https://docs.netapp.com/us-en/ontap-cli//network-interface-service-policy-create.html["création de la stratégie de service de l'interface réseau"] commandes.
...
----
+
Pour plus d'informations sur cette commande, reportez-vous au link:https://docs.netapp.com/us-en/ontap-cli/security-login-rest-role-create.html["sécurité login rest-role créer"] commande.
