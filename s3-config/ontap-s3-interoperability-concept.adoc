...
* FlexClone technology (volumes, fichiers et LUNs)
...
* Politiques QoS imbriquées
+
link:../performance-admin/policy-group-assignment-restrictions.html["Restrictions d'attribution des groupes de politiques QoS"]
...
Ou des caractéristiques supplémentaires
 a| 
* Le déplacement de volumes est pris en charge pour les volumes membres individuels qui composent un volume FlexGroup.
...
* Niveau cloud FabricPool (link:../s3-config/index.html["S3 natif uniquement"])
* Niveau local FabricPool (link:../s3-multiprotocol/index.html["Volumes NAS uniquement"])
* link:../flexcache/supported-unsupported-features-concept.html#supported-features["Volumes FlexCache"] (à partir de ONTAP 9.18.1)

 a| 
* Le déplacement de volumes n'est pas pris en charge pour les volumes FlexGroup utilisés par les compartiments S3.
* FPolicy
* Qtrees
* Quotas
