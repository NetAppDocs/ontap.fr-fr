...
ONTAP S3 prend en charge cet en-tête supplémentaire :
...
| DeleteBuckeLifecycle | ONTAP 9.8 
...
À partir d'ONTAP 9.19.1, ONTAP S3 prend en charge l'application des écritures conditionnelles pour les chargements d'objets et des suppressions conditionnelles pour les suppressions d'objets. Les écritures et suppressions conditionnelles offrent un moyen d'éviter les écrasements ou les suppressions non intentionnelles d'objets. Les opérations conditionnelles fonctionnent uniquement si la requête est effectuée avec la méthode de signature AWS_V4 via une connexion TLS. Les en-têtes conditionnels et les opérations sur les objets suivants sont pris en charge :

|===
| Opération d'objet | En-tête conditionnel 
...
ONTAP S3 prend en charge ce paramètre de requête URI supplémentaire :
`uploadId`
 a| 
ONTAP 9.8
...
ONTAP S3 prend en charge les en-têtes supplémentaires suivants :
...
| DeleteObjects ONTAP S3 prend en charge cet en-tête supplémentaire : * `x-amz-bypass-governance-retention` | ONTAP 9.11.1 
...
ONTAP S3 prend en charge les paramètres de requête URI supplémentaires suivants :
...
ONTAP S3 prend en charge les paramètres URI supplémentaires suivants :
...
* ONTAP S3 prend désormais en charge la signature des requêtes de téléchargement par morceaux à l'aide de `x-amz-content-sha256:
