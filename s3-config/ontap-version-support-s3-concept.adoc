...
Comme pour d'autres protocoles tels que FC, iSCSI, NFS, NVMe_oF et SMB, S3 nécessite l'installation d'une licence avant de pouvoir être utilisé dans ONTAP. La licence S3 est une licence gratuite, mais elle doit être installée sur les systèmes mis à niveau vers ONTAP 9.8. La licence S3 peut être téléchargée depuis le link:https://mysupport.netapp.com/site/systems/master-license-keys/ontaps3["page des clés de licence"^] sur le site de support NetApp.
