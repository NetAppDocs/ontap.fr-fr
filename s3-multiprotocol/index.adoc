...
Les « compartiments » NAS S3 sont simplement des mappages des données NAS pour les clients S3, ils ne sont pas des compartiments S3 standard. Par conséquent, il n'est pas nécessaire de protéger les compartiments NAS S3 à l'aide de la fonctionnalité NetApp S3 SnapMirror. À la place, vous pouvez répliquer les SVM source contenant des compartiments NAS S3 à l'aide de SVM DR, une relation de protection des données SnapMirror standard avec les SVM de destination. SVM DR est la seule méthode de réplication SnapMirror prise en charge avec S3 multiprotocole. SnapMirror synchrone n'est pas pris en charge.
