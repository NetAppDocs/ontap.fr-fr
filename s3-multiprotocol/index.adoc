...
NOTE: Les objets multipart dans les compartiments S3 NAS doivent être alignés sur des tailles entières, pas partielles. Par exemple, une partie peut faire 4MB ou 4GB ou une taille similaire. Une partie ne peut pas utiliser de tailles partielles ou inférieures à 1MB, telles que 4.5MB ou 4000.5MB.
...
* CopyObject non pris en charge dans les volumes SnapLock
...
* À partir d' ONTAP 9.16.1, prise en charge des métadonnées avec des objets à partie unique.
...
* À partir d' ONTAP 9.16.1, prise en charge des balises avec des objets à partie unique.
