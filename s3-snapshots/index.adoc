...
Les instantanés S3 ne capturent que les versions actuelles des objets dans les compartiments S3. Les versions non actuelles des compartiments versionnés ne sont pas capturées dans les instantanés S3. De plus, les balises d'objet à un instant donné ne sont pas capturées dans les instantanés si les balises d'objet sont modifiées après la prise des instantanés.
...
Vous devez prendre connaissance des exclusions de fonctionnalités et des scénarios suivants :
...
* Les instantanés S3 ne sont pas recommandés sur les compartiments utilisés comme niveau de capacité FabricPool local ou distant.
