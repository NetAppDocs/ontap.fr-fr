...
* Le basculement et la migration de LIF iSCSI se produisent au sein du broadcast domain of the home port de la LIF. Vérifiez la configuration de votre domaine de diffusion à l'aide du `network port broadcast-domain show` commande.
+

IMPORTANT: Assurez-vous que les domaines de diffusion associés aux LIFs iSCSI contiennent des ports réseau représentant les deux nœuds d'une paire HA, de sorte que les LIFs iSCSI puissent être migrées et basculées entre les deux nœuds selon les besoins.
