...
summary: 'Les groupes d"initiateurs sont des tables de noms WWPN d"hôtes de protocole FC ou de noms de nœuds d"hôtes iSCSI. Vous pouvez définir des groupes d"initiateurs et les associer à des LUN pour contrôler quels initiateurs ont accès aux LUN.' 
---
= Découvrez les igroups SAN ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
