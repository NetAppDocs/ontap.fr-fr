...
summary: 'ONTAP prend en charge certaines API vStorage VMware pour les fonctionnalités d"intégration de baies lorsque l"hôte ESX exécute ESX 4.1 ou une version ultérieure. Ces fonctionnalités permettent de décharger des opérations de l"hôte ESX vers le système de stockage et d"augmenter le débit réseau. L"hôte ESX active automatiquement ces fonctionnalités dans l"environnement approprié.' 
---
