...
summary: 'Le transfert de données déchargé de Microsoft, également appelé déchargement de copie, permet des transferts de données directs au sein d"un périphérique de stockage ou entre des périphériques de stockage compatibles sans transférer les données via l"ordinateur hôte.' 
---
...
Lorsque vous utilisez VMware et le déchargement de copie Microsoft dans des environnements virtualisés, vos LUN doivent être alignés.  Des LUN non alignées peuvent dégrader les performances. link:io-misalignments-properly-aligned-luns-concept.html["Apprenez-en davantage sur les LUN non alignées"].
