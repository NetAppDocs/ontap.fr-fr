...
summary: 'Vous pouvez déplacer un LUN entre volumes au sein d"une même machine virtuelle de stockage, mais vous ne pouvez pas déplacer un LUN entre SVM. Les LUN déplacés entre volumes au sein d"une SVM sont transférés instantanément et sans perte de connectivité.' 
---
= Déplacer une LUN
:allow-uri-read: 
:icons: font
