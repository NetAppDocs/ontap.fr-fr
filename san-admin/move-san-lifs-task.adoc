...
summary: 'Si un nœud doit être mis hors ligne, vous pouvez déplacer une SAN LIF afin de préserver ses informations de configuration, telles que son WWPN, et éviter le rezonage de la matrice de commutation. Étant donné qu"une SAN LIF doit être mise hors ligne avant d"être déplacée, le trafic hôte doit s"appuyer sur un logiciel de multipathing hôte pour garantir un accès non interrompu à la LUN. Vous pouvez déplacer les SAN LIF vers n"importe quel nœud d"un cluster, mais vous ne pouvez pas déplacer les SAN LIF entre des machines virtuelles de stockage.' 
---
