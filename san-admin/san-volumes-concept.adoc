...
summary: 'ONTAP propose trois options de provisionnement de volumes : le provisionnement épais, le provisionnement fin et le provisionnement semi-épais. Chaque option utilise différentes méthodes pour gérer l’espace du volume et les besoins en espace pour les technologies de partage de blocs ONTAP. Comprendre le fonctionnement de ces options vous permet de choisir la meilleure option pour votre environnement.' 
---
