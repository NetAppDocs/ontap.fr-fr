...
summary: 'Le mappage LUN sélectif réduit le nombre de chemins d"accès entre l"hôte et le LUN. Avec le mappage LUN sélectif, lorsqu"un nouveau mappage LUN est créé, le LUN n"est accessible que via les chemins situés sur le nœud propriétaire du LUN et son partenaire HA.' 
---
