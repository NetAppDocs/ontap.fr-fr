...
summary: 'Votre serveur SMB prend en charge les objets de stratégie de groupe (GPO), un ensemble de règles appelées attributs de stratégie de groupe qui s"appliquent aux ordinateurs d"un environnement Active Directory. Vous pouvez utiliser les GPO pour gérer de manière centralisée les paramètres de toutes les machines virtuelles de stockage du cluster appartenant au même domaine Active Directory.' 
---
