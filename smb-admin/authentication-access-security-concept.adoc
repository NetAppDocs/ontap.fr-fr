...
summary: 'Avant que les utilisateurs puissent créer des connexions SMB pour accéder aux données contenues sur la SVM, ils doivent être authentifiés par le domaine auquel appartient le serveur SMB. Le serveur SMB prend en charge deux méthodes d"authentification, Kerberos et NTLM. Kerberos est la méthode par défaut utilisée pour authentifier les utilisateurs du domaine.' 
---
