...
[NOTE]
====
Microsoft ne recommande plus d'utiliser  `Digitally sign communications (if client agrees)` ou  `Digitally sign communications (if server agrees)` les paramètres de stratégie de groupe. Microsoft ne recommande plus non plus d'utiliser les paramètres du registre  `EnableSecuritySignature`. Ces options n'affectent que le comportement SMB 1 et peuvent être remplacées par le paramètre de stratégie de groupe  `Digitally sign communications (always)` ou le paramètre du registre  `RequireSecuritySignature`. Vous pouvez également obtenir plus d'informations à partir de link:http://blogs.technet.com/b/josebda/archive/2010/12/01/the-basics-of-smb-signing-covering-both-smb1-and-smb2.aspx["Blog Microsoft : Les bases de la signature SMB (couvrant SMB1 et SMB2)"^].
