...
summary: 'À partir d"ONTAP 9.3, vous pouvez modifier le processus par défaut de découverte des contrôleurs de domaine. Cela vous permet de limiter la découverte à votre site ou à un ensemble de contrôleurs de domaine privilégiés, ce qui peut améliorer les performances selon l"environnement.' 
---
...
+
Le `default-site` paramètre du serveur SMB peut être défini pour utiliser ce mode avec des LIF qui ne sont pas affectées à un site dans sites-and-services. À partir d'ONTAP 9.13.1, le site par défaut peut également être utilisé comme solution de secours si ONTAP ne peut pas découvrir les informations de site pour un domaine de confiance.
