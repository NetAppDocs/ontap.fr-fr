...
---
= Activer BranchCache sur les nouveaux partages SMB
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Étape
. Créer un partage SMB compatible BranchCache :
+
[source, cli]
----
vserver cifs share create -vserver <SVM_name> -share-name <share_name> -path <path> -share-properties branchcache[,...]
----
. Vérifiez que la propriété de partage BranchCache est définie sur le partage SMB :
+
[source, cli]
----
