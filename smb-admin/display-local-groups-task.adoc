...
summary: 'Vous pouvez afficher la liste de tous les groupes locaux configurés sur le cluster ou sur une machine virtuelle de stockage spécifique. Ces informations peuvent s"avérer utiles lors du dépannage des problèmes d"accès aux fichiers aux données contenues sur la SVM ou des problèmes de droits des utilisateurs sur la SVM.' 
---
