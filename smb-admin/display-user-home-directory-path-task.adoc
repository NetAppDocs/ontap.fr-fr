...
summary: 'Vous pouvez afficher le chemin d"accès au répertoire local d"un utilisateur SMB sur la machine virtuelle de stockage, ce qui peut être utile si vous avez plusieurs chemins d"accès au répertoire local CIFS configurés et que vous souhaitez voir quel chemin contient le répertoire local de l"utilisateur.' 
---
