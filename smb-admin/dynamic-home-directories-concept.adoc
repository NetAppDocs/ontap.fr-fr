...
summary: 'Les répertoires locaux ONTAP permettent de configurer un partage SMB qui pointe vers différents répertoires selon l"utilisateur qui s"y connecte et un ensemble de variables. Au lieu de créer des partages distincts pour chaque utilisateur, vous pouvez configurer un seul partage avec quelques paramètres de répertoire local afin de définir la relation d"un utilisateur entre un point d"entrée et le répertoire local.' 
---
