...
summary: 'Vous pouvez activer un compte d"utilisateur local si vous souhaitez que cet utilisateur puisse accéder aux données contenues dans la machine virtuelle de stockage via une connexion SMB. Vous pouvez également désactiver un compte d"utilisateur local si vous ne souhaitez pas que cet utilisateur accède aux données de la machine virtuelle de stockage via SMB.' 
---
