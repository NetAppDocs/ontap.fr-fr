...
summary: 'Vous pouvez activer ou désactiver l"authentification de l"utilisateur local pour l"accès SMB aux machines virtuelles de stockage. Par défaut, l"authentification de l"utilisateur local est autorisée, ce qui est utile lorsque la machine virtuelle de stockage ne peut pas contacter un contrôleur de domaine ou si vous choisissez de ne pas utiliser les règles d"accès au niveau du domaine.' 
---
