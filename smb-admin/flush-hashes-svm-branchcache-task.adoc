...
summary: Vous pouvez vider tous les hachages mis en cache du magasin de hachages BranchCache sur la machine virtuelle de stockage. Cela peut être utile si vous avez modifié la configuration BranchCache du bureau distant. Par exemple, si vous avez récemment reconfiguré le mode de mise en cache de la mise en cache distribuée vers le mode de mise en cache hébergée, vous voudrez vider le magasin de hachages. 
---
