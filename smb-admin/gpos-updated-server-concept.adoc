...
summary: 'Par défaut, ONTAP récupère et applique les modifications des objets de stratégie de groupe (GPO) toutes les 90 minutes. Les paramètres de sécurité sont actualisés toutes les 16 heures. Si vous souhaitez mettre à jour les GPO pour appliquer les nouveaux paramètres de stratégie avant qu"ONTAP ne les mette à jour automatiquement, vous pouvez déclencher une mise à jour manuelle sur un serveur CIFS avec une commande ONTAP.' 
---
