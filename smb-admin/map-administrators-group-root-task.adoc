...
summary: 'Si votre environnement ne comporte que des clients CIFS et que votre machine virtuelle de stockage est configurée comme un système de stockage multiprotocole, vous devez disposer d"au moins un compte Windows avec privilèges root pour accéder aux fichiers sur la SVM. Sinon, vous ne pouvez pas gérer la SVM car vous ne disposez pas de droits d"utilisateur suffisants.' 
---
