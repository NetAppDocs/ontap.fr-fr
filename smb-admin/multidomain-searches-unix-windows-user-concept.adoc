...
À partir de ONTAP 9.13.1, lorsque ONTAP recherche des domaines de confiance lors des opérations de mappage de noms multidomaines, il effectue une découverte de site pour ces domaines de confiance et utilise le site découvert pour les recherches de contrôleurs de domaine. Ce comportement permet de garantir que les requêtes DNS SRV utilisent les informations de site correctes pour les contrôleurs de domaine des domaines de confiance. Si la découverte de site des domaines de confiance échoue, ONTAP utilise le site par défaut du serveur SMB.
