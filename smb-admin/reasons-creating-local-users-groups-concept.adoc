...
summary: 'Il existe plusieurs raisons de créer des utilisateurs et des groupes locaux sur votre machine virtuelle de stockage. Par exemple, vous pouvez accéder à un serveur SMB à l"aide d"un compte d"utilisateur local si les contrôleurs de domaine sont indisponibles, vous pouvez utiliser des groupes locaux pour attribuer des privilèges, ou votre serveur SMB se trouve dans un groupe de travail.' 
---
