...
summary: 'Il existe quatre styles de sécurité différents : UNIX, NTFS, mixte et unifié. Chaque style de sécurité a un effet différent sur la façon dont les permissions sont gérées pour les données. Vous devez comprendre les différents effets afin de vous assurer de sélectionner le style de sécurité approprié pour vos besoins.' 
---
...
| NTFS | PME | ALC NTFS | NTFS | NFS et SMB 
...
* ALC NTFS
 ...
 a| 
* UNIX (pour les bits de mode NFSv3 et les ACL NFSv4.x)
* NTFS (pour les ACL NTFS)

| NFS et SMB 


| Unifiée
(Pour Infinite volumes uniquement, dans ONTAP 9.4 et les versions antérieures.) | NFS ou SMB  a| 
* Bits de mode NFSv3
* ACL NFSv4.1
* ALC NTFS

 a| 
* UNIX (pour les bits de mode NFSv3 et les ACL NFSv4.1)
* NTFS (pour les ACL NTFS)
...
NOTE: Les styles de sécurité peuvent être définis sur les volumes FlexVol (racine ou données) et les qtrees. Les styles de sécurité peuvent être définis manuellement lors de la création, hérités automatiquement ou modifiés ultérieurement. == Décidez quels styles de sécurité SMB utiliser sur les SVM ONTAP
