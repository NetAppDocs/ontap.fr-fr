...
+
La fonction de transfert direct de copie ODX prend en charge les opérations de copie initiées depuis un système d’exploitation invité Hyper-V. Le système d’exploitation invité doit prendre en charge ODX et utiliser des disques virtuels SCSI reposant sur un stockage SMB ou SAN compatible ODX. Les disques virtuels IDE ne prennent pas en charge le transfert direct ODX. Le stockage source et le stockage de destination doivent se trouver sur le même cluster.

+
Pour plus d'informations, consultez link:use-cases-odx-concept.html["Cas d'utilisation d'ODX sur les serveurs ONTAP SMB"].
