...
summary: 'Lorsqu"une application de sauvegarde exécutée sous Windows rencontre un lien symbolique de type Unix, le lien est suivi et les données sont sauvegardées. À partir d"ONTAP 9.15.1, vous avez la possibilité de sauvegarder les liens symboliques au lieu des données. Cette fonctionnalité est entièrement prise en charge avec les volumes FlexGroup et FlexVols d"ONTAP.' 
---
...
En plus de l'interface de ligne de commande ONTAP, vous pouvez également gérer cette fonctionnalité à l'aide de l'API REST ONTAP. Voir  https://docs.netapp.com/us-en/ontap-automation/whats-new.html["Nouveautés de l'API REST ONTAP et de l'automatisation"^] pour plus d'informations. La configuration déterminant la façon dont ONTAP traite les liens symboliques de type Unix doit être effectuée séparément pour chaque SVM.
