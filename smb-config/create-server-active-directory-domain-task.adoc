...
Vous devez disposer des privilèges minimaux suivants sur l'unité organisationnelle (OU) pour créer le serveur SMB :

* Créer des objets informatiques
* Supprimer les objets d'ordinateur
* Réinitialiser le mot de passe
* Restrictions de lecture et d'écriture du compte
* Écriture validée sur nom de l'hôte DNS
* Écriture validée vers les service principal names
* Lire msDS-SupportedEncryptedTypes
* Écrivez msDS-SupportedEncryptedTypes
...
* Vous pouvez choisir d'ajouter le serveur SMB à une autre unité organisationnelle en utilisant l'option  `-ou`.
* À partir d'ONTAP 9.13.1, vous pouvez configurer un site par défaut pour le serveur SMB. ONTAP peut utiliser ce site lorsqu'une LIF n'est pas membre d'un site et comme solution de repli si la découverte du site échoue pour un domaine approuvé lors de la localisation des contrôleurs de domaine du domaine approuvé.
...
. Créez le serveur SMB dans un domaine AD `+vserver cifs create -vserver vserver_name -cifs-server smb_server_name -domain FQDN [-ou organizational_unit][-default-site site_name][-netbios-aliases NetBIOS_name, ...][-keytab-uri {(ftp|http)://hostname|IP_address}][-comment text]+` :
+
...
À partir d'ONTAP 9.13.1, le site par défaut du serveur SMB est également utilisé comme solution de repli si ONTAP ne parvient pas à découvrir les informations de site pour un domaine approuvé.
