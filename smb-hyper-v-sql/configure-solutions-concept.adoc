...
summary: Vous pouvez utiliser des partages de fichiers SMB 3.0 et versions ultérieures disponibles en permanence pour stocker les fichiers de machines virtuelles Hyper-V ou les bases de données système et utilisateur SQL Server sur des volumes résidant dans des SVM, tout en assurant des opérations non perturbatrices pour les événements planifiés et non planifiés. 
---
...
* NetApp Backup and Recovery protège les machines virtuelles Hyper-V autonomes, les instances de cluster FCI et les machines virtuelles provisionnées par System Center Virtual Machine Manager (SCVMM) hébergées sur des partages CIFS.
+
Pour plus d'informations sur la protection des charges de travail Hyper-V avec NetApp Backup and Recovery, consultez le https://docs.netapp.com/us-en/data-services-backup-recovery/br-use-hyperv-protect-overview.html["Vue d'ensemble de la protection des charges de travail Hyper-V"].
