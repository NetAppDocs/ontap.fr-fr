...
summary: Vous devez créer des volumes de données NTFS sur la machine virtuelle de stockage avant de pouvoir configurer des partages à disponibilité continue pour une utilisation avec Hyper-V ou SQL Server sur des serveurs applicatifs SMB. Utilisez la feuille de calcul de configuration des volumes pour créer vos volumes de données. 
---
