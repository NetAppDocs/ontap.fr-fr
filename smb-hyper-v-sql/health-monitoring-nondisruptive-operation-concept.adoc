...
summary: 'Le contrôle de l"état fournit des informations sur l"état de santé du système à travers le cluster. Le contrôle de l"état surveille les configurations Hyper-V et SQL Server sur SMB afin de garantir des opérations sans interruption pour les serveurs applicatifs. En cas de dégradation de l"état, vous pouvez consulter les détails du problème, notamment la cause probable et les actions de récupération recommandées.' 
---
