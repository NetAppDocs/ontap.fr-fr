...
[role="lead"]
L'ajout et la suppression de volumes dans une relation SM-BC active dépendent de la version de ONTAP que vous utilisez.
...
Depuis ONTAP 9.10.1, il est recommandé de le gérer link:../consistency-groups/index.html["groupes de cohérence"] Via System Manager ou avec l'API REST ONTAP. Si vous souhaitez modifier la composition du groupe de cohérence en ajoutant ou en supprimant un volume, vous devez d'abord supprimer la relation d'origine, puis créer à nouveau le groupe de cohérence avec la nouvelle composition.
...
NOTE: La suppression de volumes d'un groupe de cohérence avec une relation SM-BC engendre des perturbations et vous devez interrompre la relation SnapMirror avant de poursuivre cette opération.
