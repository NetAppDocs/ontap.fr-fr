...
À partir de ONTAP 9.14.1, le clustering avec basculement Windows est pris en charge par SM-BC. Pour plus d'informations, voir link:https://www.netapp.com/pdf.html?item=/media/21888-tr-4878.pdf["Tr-4878 : continuité de l'activité avec SnapMirror"^].
...
Si vous rencontrez un basculement sur la destination SM-BC dans une configuration de « Fan-Out », vous devez le faire manuellement xref:smbc_admin_what_happens_during_an_automatic_unplanned_failover.html#resume-protection-in-a-fan-out-configuration-after-failover[reprendre la protection dans la configuration du « fan-out »].
