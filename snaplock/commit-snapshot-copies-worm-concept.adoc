...
. Sur le cluster de destination, installez la licence SnapLock, initialisez l'horloge de conformité et, si vous utilisez une version ONTAP antérieure à la version 9.10.1, créez un agrégat SnapLock, comme décrit dans xref:workflow-concept.html[Workflow SnapLock].
