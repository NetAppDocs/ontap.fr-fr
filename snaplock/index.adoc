...
En utilisant SnapLock, vous engagez des fichiers et des instantanés sur un stockage WORM et définissez des périodes de rétention pour les données protégées par WORM. Le stockage WORM SnapLock utilise la technologie des instantanés NetApp et peut tirer parti de la réplication SnapMirror et des sauvegardes SnapVault comme technologies de base pour assurer la protection de la restauration des données. En savoir plus sur le stockage WORM : link:https://www.netapp.com/pdf.html?item=/media/6158-tr4526.pdf["Conformité du stockage WORM avec NetApp SnapLock - TR-4526"^].
