...
---
= Mettez en miroir les fichiers WORM avec ONTAP SnapMirror pour la reprise après sinistre
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Une relation SnapMirror d'un volume SnapLock prend uniquement en charge `MirrorAllSnapshots` politique de type async-mirror.  La période de rétention d'un volume SnapLock est déterminée par la période de rétention maximale parmi tous les fichiers WORM qu'il contient.  Étant donné que la destination est une copie DR de la source, la période de conservation du volume SnapLock de destination sera la même que celle de la source.
...
NOTE: À partir d' ONTAP 9.10.1, les volumes SnapLock et non SnapLock peuvent exister sur le même agrégat ; par conséquent, vous n'êtes plus obligé de créer un agrégat SnapLock distinct si vous utilisez ONTAP 9.10.1.  Vous utilisez l’option volume -snaplock-type pour spécifier un type de volume SnapLock de conformité ou d’entreprise.  Dans les versions ONTAP antérieures à ONTAP 9.10.1, le mode SnapLock (Conformité ou Entreprise) est hérité de l'agrégat.  Le paramètre de langue du volume de destination doit correspondre au paramètre de langue du volume source.
