...
---
= Définir la durée de rétention ONTAP SnapLock
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
ONTAP vous empêche de rétablir un cluster d' ONTAP 9.10.1 vers une version antérieure ONTAP lorsqu'il existe des fichiers avec une période de conservation postérieure au « 19 janvier 2071 8:44:07 AM ».
...
IMPORTANT: Dans les versions antérieures à ONTAP 9.10.1, si vous ne définissez pas explicitement la durée de conservation avant de valider un fichier en mode de conformité dans l'état WORM, et que vous ne modifiez pas les valeurs par défaut, le fichier sera conservé pendant 30 ans. Cette modification est irréversible. De même, dans ONTAP 9.10.1 et versions ultérieures, si vous ne définissez pas explicitement la durée de conservation avant de valider un fichier en mode Entreprise dans l'état WORM, et que vous ne modifiez pas les valeurs par défaut, le fichier sera conservé pendant 0 an, ou, en réalité, pas du tout.
...
====
Pour une utilisation avancée, voir link:https://www.netapp.com/pdf.html?item=/media/6158-tr4526.pdf["Stockage WORM conforme avec NetApp SnapLock"^].
