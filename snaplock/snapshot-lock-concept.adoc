...
* Volumes d'origine FlexCache®
+
Pris en charge à partir de ONTAP 9.16.1.

* SnapMirror synchrone
+
À partir d'ONTAP 9.19.1, vous pouvez spécifier la durée de validité uniquement lors de la création d'un instantané. Il n'est pas possible de définir ou de prolonger la durée de validité des instantanés existants.
...
. Enregistrez les modifications.
. Dans la fenêtre *volumes*, sélectionnez le volume que vous avez mis à jour et choisissez *vue d'ensemble*.
. Vérifiez que * Verrouillage SnapLock Snapshot* s'affiche comme *Activé*.
