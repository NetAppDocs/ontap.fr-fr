...
summary: 'Vous pouvez utiliser les commandes de démarrage des empreintes du fichier de volume et de vidage des empreintes du fichier de volume pour afficher les principales informations relatives aux fichiers et volumes, y compris le type de fichier (régulier, WORM ou WORM apppliable), la date d"expiration du volume, etc.' 
---
= Vérifier les paramètres ONTAP SnapLock
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Vous pouvez utiliser le `volume file fingerprint start` et `volume file fingerprint dump` Commandes permettant d'afficher des informations clés sur les fichiers et volumes, y compris le type de fichier (standard, WORM ou WORM appliable), la date d'expiration du volume, etc.

.Étapes
. Générer une empreinte de fichier :
+
...
====
. Afficher l'empreinte du fichier :
+
