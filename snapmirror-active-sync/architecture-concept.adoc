...
Dans une configuration SAN, la relation de protection des données assurant la continuité d'activité est créée entre le système de stockage source et le système de stockage de destination, en ajoutant les LUN ou espaces de noms NVMe spécifiques à l'application, provenant de différents volumes au sein d'une machine virtuelle de stockage (SVM), au groupe de cohérence. En fonctionnement normal, l'application d'entreprise écrit dans le groupe de cohérence principal, qui réplique de manière synchrone ces E/S dans le groupe de cohérence miroir.

image:snapmirror-active-sync-architecture.png["Architecture de SnapMirror actif"]

Bien que deux copies distinctes des données existent dans la relation de protection des données, parce que SnapMirror active sync conserve la même identité LUN ou espace de noms NVMe dans une configuration SAN, et les mêmes volumes dans un SVM pour une configuration NAS, l’hôte de l’application perçoit cela comme un périphérique virtuel partagé avec plusieurs chemins d’accès, alors qu’une seule copie de LUN, d’espace de noms NVMe ou de volume est écrite à la fois. Lorsqu’une panne met le système de stockage principal hors ligne, ONTAP détecte cette panne et utilise le Mediator pour une reconfirmation ; si ni ONTAP ni le Mediator ne parviennent à joindre le site principal, ONTAP effectue l’opération de basculement automatique. Ce processus permet de basculer uniquement une application spécifique, sans intervention manuelle ni script, ce qui était auparavant nécessaire pour le basculement.

Autres points à prendre en compte :

* Les volumes sans miroir qui sont en dehors de la protection pour la continuité de l'activité sont pris en charge.
* Une seule relation de distribution asynchrone SnapMirror est prise en charge pour les volumes protégés pour la continuité d'activité. Dans les configurations NAS, seule une relation asynchrone basée sur les volumes est prise en charge. Les relations asynchrones SVM ou de groupe de cohérence ne sont pas prises en charge.
...
* Cluster ONTAP principal hébergeant la SVM principale de synchronisation active SnapMirror
* Cluster ONTAP secondaire hébergeant le SVM miroir
* Médiateur de ONTAP
...
Consultez la documentation link:mediator-install-task.html["Médiateur de ONTAP"] pour plus d'informations.
...
== SnapMirror workflow de synchronisation active pour les configurations SAN

La figure suivante illustre la conception de SnapMirror active sync dans une configuration SAN à un niveau élevé.
...
SnapMirror active sync propose des solutions asymétriques et symétriques pour les configurations SAN.
...
== Flux de travail de l'opération de synchronisation active SnapMirror pour les configurations NAS

À partir de ONTAP 9.19.1, SnapMirror active sync assure la protection des charges de travail NAS au niveau du SVM. Dans une configuration NAS, le SVM de destination (secondaire) reste en état dormant jusqu'à la survenue d'un événement de basculement. Pour permettre un basculement sans interruption, l'identité réseau et les configurations de protocole, y compris les adresses IP, doivent être identiques sur les SVM principal et secondaire. Ces configurations sont automatiquement répliquées vers le cluster secondaire, garantissant une cohérence de l'adressage IP et des paramètres de protocole entre les deux clusters. La figure suivante illustre la conception de SnapMirror active sync dans une configuration NAS à un niveau global.

image:snapmirror-active-sync-nas-config.png["Configuration NAS de synchronisation active SnapMirror"]

Dans ONTAP 9.19.1, seuls les volumes FlexVol sont pris en charge. Les groupes de cohérence ne sont pas pris en charge. Avec la prise en charge uniquement de FlexVol, le cluster secondaire ne garantit pas la cohérence de l'ordre d'écriture entre les volumes. Sur la figure, les données et les informations de configuration sont répliquées de la SVM A1 du cluster source vers la SVM inactive, SVM A1', du cluster de destination.
