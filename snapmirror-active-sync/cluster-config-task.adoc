...
. Au moins une LIF SAN (FC ou iSCSI selon le cas) est créée sur chaque nœud dans les deux clusters pour votre configuration SAN, ou au moins une LIF NAS (NFS ou SMB selon le cas) est créée sur chaque nœud dans le cluster principal pour la réplication vers le cluster secondaire.
+
...
+
link:../nfs-config/index.html["Découvrez la configuration NFS"]

+
link:../smb-config/index.html["Découvrez la configuration SMB"]

. SAN uniquement : les LUN nécessaires sont créés et mappés à un igroup, qui est utilisé pour mapper les LUN à l’initiateur sur l’hôte de l’application.
+
link:../san-admin/provision-storage.html["Créer des LUN et mapper des igroups"]

. L'hôte de l'application est analysé à nouveau afin de détecter tout nouveau LUN ou volume.
