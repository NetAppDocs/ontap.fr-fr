...
---
= Convertir une relation SAN ONTAP SnapMirror existante en une relation SnapMirror active sync pour SAN
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Si vous avez configuré la protection SnapMirror, vous pouvez convertir une relation SAN en synchronisation active SnapMirror pour SAN. À partir de ONTAP 9.15.1, vous pouvez convertir la relation pour utiliser une protection symétrique active/active.

Pour plus d'informations sur la conversion d'une relation SVM-DR en une relation de synchronisation active SnapMirror pour NAS, voir link:protect-nas-task.html#convert-an-svm-dr-relationship-to-a-snapmirror-active-sync-relationship["Convertir une relation SVM DR en une relation de synchronisation active SnapMirror"].
