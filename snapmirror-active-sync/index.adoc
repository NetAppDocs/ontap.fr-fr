...
Dans les configurations asymétriques actives/passives, les données sur le site secondaire sont acheminées par proxy vers un LUN, un espace de noms ou une unité de stockage.
...
Les modes « primaire en premier » et « biais primaire » sont pris en charge dans SnapMirror active sync à partir de ONTAP 9.15.1. Les copies primaires sont désignées dans System Manager et affichées avec l'API REST ONTAP et l'interface de ligne de commande ONTAP.
...
* NVMe pour les charges de travail VMware
 ...
 a| 
* Asymétrique actif/passif



NOTE: L'architecture asymétrique active/passive ne prend pas en charge ASA r2 et NVMe. Pour plus d'informations sur la prise en charge de NVMe, consultez link:../nvme/support-limitations.html["Configuration, prise en charge et limitations de NVMe"].
