...
À partir d'ONTAP 9.15.1, SnapMirror active sync prend en charge la reconfiguration automatique dans la branche de distribution après un événement de basculement. Si le basculement du site principal vers le site secondaire a réussi, le site tertiaire est automatiquement reconfiguré pour considérer le site secondaire comme la source. La branche de distribution asynchrone peut être une relation de groupe de cohérence dans une configuration SAN ou une relation de volume indépendante. La reconfiguration fonctionne dans les deux cas. La reconfiguration est déclenchée par un basculement planifié ou non planifié. La reconfiguration a également lieu lors du retour au site principal.
