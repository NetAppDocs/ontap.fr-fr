...
Pour plus d'informations sur la prise en charge des configurations NAS, consultez link:protect-nas-task.html["Protégez avec ONTAP SnapMirror active sync pour NAS"].
...
* AIX
+
** À partir d'ONTAP 9.19.1, le basculement transparent des applications (TAF) est pris en charge avec les configurations symétriques actives/actives. Aucun PVR n'est requis.
** À partir d'ONTAP 9.11.1 jusqu'à ONTAP 9.18.1, AIX est uniquement pris en charge par PVR pour le basculement automatique avec des configurations symétriques actives/actives.
...
À partir d'ONTAP 9.19.1, le basculement transparent des applications pour AIX est pris en charge avec la synchronisation active SnapMirror dans les configurations symétriques actif/actif. Vous pouvez utiliser System Manager ou l'interface de ligne de commande ONTAP pour créer un LUN AIX. Pour garantir un basculement transparent, lorsque vous utilisez l'interface de ligne de commande ONTAP, vous devez utiliser l'option access-mode local avec la commande lun create ou lun modify. Par défaut, le mode d'accès est défini sur global. Lorsque vous utilisez System Manager, choisissez AIX comme hôte et local comme mode d'accès. Pour plus d'informations sur la création d'un LUN, consultez link:../san-admin/provision-storage.html["Provisionnement du stockage SAN"]. Pour plus d'informations sur la prise en charge d'AIX, consultez l'article de la base de connaissances NetApp : link:https://kb.netapp.com/on-prem/ontap/DP/SnapMirror/SnapMirror-KBs/SnapMirror_active_sync_support_for_AIX%3A_behavior_by_ONTAP_release_and_migration_guidance["SnapMirror active sync prise en charge pour AIX : comportement selon la version d'ONTAP et guide de migration"^].

Si vous utilisez ONTAP 9.11.1 à ONTAP 9.18.1, AIX est pris en charge avec SnapMirror active sync via une demande de dérogation à la politique de fonctionnalités d'ingénierie standard (FPVR), sous réserve de l'acceptation des stipulations suivantes :
...
Un basculement automatique non planifié (AUFO) sur le cluster principal isolé peut être provoqué par une double défaillance lorsque la connexion entre le cluster principal et le cluster secondaire est perdue, et que la connexion entre le cluster principal et le médiateur est également perdue. Il s'agit d'un événement rare, contrairement aux autres événements AUFO.
...
À partir d'ONTAP 9.19.1, ARP/AI prend en charge les volumes principaux qui participent aux relations SAN de synchronisation active SnapMirror. Pour plus d'informations, consultez link:../anti-ransomware/use-cases-restrictions-concept.html#interoperability-and-vm-considerations["Cas d'utilisation et considérations relatives à la protection autonome contre les ransomwares"].
...
SnapMirror active sync pour SAN prend en charge xref:../data-protection/supported-deployment-config-concept.html[configurations « fan-out »] avec la `MirrorAllSnapshots` stratégie et, à partir de ONTAP 9.11.1, la `MirrorAndVault` stratégie. Les configurations en éventail ne sont pas prises en charge dans SnapMirror active sync avec la `XDPDefault` stratégie.

À partir d'ONTAP 9.15.1, SnapMirror active sync prend en charge la reconfiguration automatique de la liaison de distribution après un événement de basculement. Si le basculement du site principal vers le site secondaire a réussi, la reconfiguration de la liaison de distribution est lancée lors de la prochaine mise à jour planifiée de la liaison asynchrone de distribution. Elle n'est pas déclenchée immédiatement après le basculement de la synchronisation active SnapMirror. La liaison asynchrone de distribution peut être une relation de groupe de cohérence dans une configuration SAN ou une relation de volume indépendante. La reconfiguration fonctionne dans les deux cas. La reconfiguration est déclenchée par un basculement planifié ou non planifié. La reconfiguration a également lieu lors d'un retour au site principal.
...
À partir de ONTAP 9.12.1, link:../data-protection/restore-part-file-snapshot-task.html["Restauration partielle de LUN"] est pris en charge pour les volumes SnapMirror active sync. Vous pouvez restaurer des données à partir d’instantanés créés par des applications qui ont été répliqués entre le volume source SnapMirror active sync et les volumes de destination (instantané). Une restauration partielle de LUN ou de fichier peut être nécessaire si vous devez restaurer une base de données sur un hôte qui stocke plusieurs bases de données sur le même LUN. L’utilisation de cette fonctionnalité requiert que vous connaissiez le décalage d’octet de départ des données et le nombre d’octets.
...
Les instantanés inviolables ne sont pas pris en charge avec la synchronisation active SnapMirror.
