...
---
= Configurer ONTAP Mediator et les clusters pour la synchronisation active SnapMirror
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
+
Pour plus d'informations, voir link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/task_configuring_the_ontap_mediator_service_from_a_metrocluster_ip_configuration.html["Préparez-vous à installer ONTAP Mediator"^].

* Par défaut, ONTAP Mediator fournit un service via le port TCP 31784. Vous devez vous assurer que le port 31784 est ouvert et disponible entre les clusters ONTAP et ONTAP Mediator.
