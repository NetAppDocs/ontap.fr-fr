...
Lors de l'utilisation d'ONTAP Mediator pour surveiller une relation de synchronisation active SnapMirror, ONTAP Mediator ne peut pas être initialisé dans ONTAP sans un certificat auto-signé ou un certificat d'autorité de certification (CA) valide. Vous ajoutez un certificat valide au magasin de certificats des clusters appariés.
