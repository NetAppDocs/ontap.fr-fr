...
Vous pourriez souhaiter effectuer un basculement planifié pour tester l'état de votre configuration de reprise après sinistre ou pour effectuer une maintenance sur le cluster principal.
...
. Pour surveiller l'état du basculement, utilisez la commande  `snapmirror failover show` dans l'interface de ligne de commande ONTAP.
...
+
Exemple de basculement d'une configuration SAN :

+
destination::>snapmirror failover start -destination-path vs1_dst:/cg/cg_dst

+
Exemple de basculement d'une configuration NAS :

+
destination::>snapmirror failover start -destination-path vs_b:

. Depuis la destination, surveillez la progression du basculement :
+
