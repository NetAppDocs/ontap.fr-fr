...
---
= Protégez avec ONTAP SnapMirror active sync pour SAN
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
La configuration de la protection asymétrique pour les configurations SAN utilisant SnapMirror active sync implique la sélection des LUN sur le cluster source ONTAP et leur ajout à un groupe de cohérence.
...
* Les clusters source et de destination ne peuvent pas faire partie d'une configuration MetroCluster.
...
Vous pouvez établir une protection symétrique pour les configurations SAN à l'aide de System Manager ou de l'interface de ligne de commande ONTAP. Dans les deux interfaces, il existe différentes étapes pour xref:index.html#key-concepts[configurations uniformes et non uniformes].
...
. Sur le site principal, link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes["créer un groupe de cohérence à l'aide de nouveaux LUN"].
+
