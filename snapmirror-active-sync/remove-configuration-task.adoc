...
---
= Supprimer une configuration de synchronisation active ONTAP SnapMirror
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
== Supprimer une configuration SAN asymétrique

* Avant de supprimer la relation SAN de synchronisation active SnapMirror, tous les LUNs dans le cluster de destination doivent être démappés.
...
. Dans la fenêtre *Supprimer la configuration de réplication*, cochez *Libérer les instantanés de base du volume source.* et sélectionnez *Supprimer*.
...
== Supprimer une configuration NAS
...
. Sur le cluster source, accédez à *Protection > Réplication*.
. Localisez la relation que vous souhaitez supprimer, sélectionnez image:icon_kabob.gif["Icône des options de menu"] et choisissez *Supprimer*.
. Dans la fenêtre *Supprimer la configuration de réplication*, cochez *Libérer les instantanés de base de la machine virtuelle de stockage source.* et sélectionnez *Supprimer*.
...
----
+
La SVM et ses volumes sont supprimés du cluster secondaire.
