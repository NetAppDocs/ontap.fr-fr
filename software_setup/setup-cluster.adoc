...
[role="lead"]
NetApp recommande d'utiliser System Manager pour créer de nouveaux clusters. System Manager offre un flux de travail simple et facile pour la configuration des clusters. Il n'est nécessaire d'utiliser la ligne de commandes ONTAP (CLI) que si vous exécutez ONTAP 9.7 ou une version antérieure dans une configuration MetroCluster ou si vous devez configurer un cluster IPv6 uniquement sur des systèmes pris en charge.

.Description de la tâche
Cette procédure s'applique à link:https://docs.netapp.com/us-en/ontap-systems-family/index.html["AFF, ASA, FAS"^] et link:https://docs.netapp.com/us-en/ontap-afx/index.html["Systèmes AFX"^]. Si vous avez un système ASA r2 (ASA A1K, ASA A90, ASA A70, ASA A50, ASA A30, ASA A20 ou ASA C30), suivez link:https://docs.netapp.com/us-en/asa-r2/install-setup/initialize-ontap-cluster.html["procédure à suivre"^] pour utiliser System Manager afin de configurer un cluster ONTAP. Les systèmes ASA r2 offrent une expérience ONTAP simplifiée, spécifique aux clients SAN uniquement.

L'attribution d'adresses IPv6 aux LIF de gestion lors de la configuration du cluster est prise en charge sur les systèmes et les versions ONTAP suivants.

[cols="2"]
|===
| Version ONTAP | Systèmes pris en charge 
...
Votre version de BMC doit être 19.3 ou ultérieure pour prendre en charge l'attribution d'adresses IPv6 lors de la configuration du cluster.



| ONTAP 9.13.1 et versions ultérieures  a| 
* AFF A800
* FAS8700


|===
Vous devez effectuer la configuration du cluster avec la ligne de commandes ONTAP pour attribuer des adresses IPv6 sur les systèmes pris en charge. Si l'attribution d'adresses IPv6 lors de la configuration du cluster n'est pas prise en charge sur votre système et version ONTAP, vous devez utiliser System Manager pour créer votre cluster avec des adresses IPv4, puis link:convert-ipv4-to-ipv6-task.html["Convertir en IPv6"] attribuer les adresses IPv6 après avoir terminé la configuration du cluster.
...
.Avant de commencer
* Vous devez avoir installé, câblé et mis sous tension votre nouveau système de stockage conformément aux instructions d'installation et de configuration de votre modèle de système.
+
Voir le link:https://docs.netapp.com/us-en/ontap-systems/index.html["Documentation des systèmes matériels ONTAP"^].

* Si des commutateurs de cluster font partie de l'installation du système, consultez les considérations de câblage et de configuration de votre modèle de commutateur de cluster pour les ports de cluster de vos nouveaux systèmes de stockage. Voir link:https://docs.netapp.com/us-en/ontap-systems-switches/index.html["Documentation de commutation pour les systèmes matériels ONTAP"^].
+
