...
+
Le `-application` paramètre des[`security login` commandes spécifie la méthode d'accès pour un compte utilisateur. En savoir plus sur la commande link:https://docs.NetApp.com/US-en/ONTAP-cli/Security-login-create.html#description[`security login`^] dans la référence de commande ONTAP.
