...
* À partir de ONTAP 9.19.1, les administrateurs peuvent utiliser la link:https://docs.netapp.com/us-en/ontap-cli/security-ssh-publickey-show.html["security ssh publickey show -vserver  -host-key-algorithm  command"^] pour afficher l’empreinte de la clé hôte ONTAP. Cela permet aux administrateurs de vérifier que cette empreinte correspond à celle stockée dans ONTAP.
