...
* À partir d'ONTAP 9.13.1, vous pouvez ajouter, en option, la validation de certificat au processus d'authentification SSH afin de renforcer la sécurité de la connexion. Pour ce faire, link:../authentication/manage-ssh-public-keys-and-certificates.html["Associer un certificat X.509 à la clé publique"] qu'un compte utilise. Si vous vous connectez via SSH avec une clé publique SSH et un certificat X.509, ONTAP vérifie la validité du certificat X.509 avant d'authentifier avec la clé publique SSH. La connexion SSH est refusée si ce certificat est expiré ou révoqué, et la clé publique SSH est automatiquement désactivée.
* À partir d'ONTAP 9.14.1, les administrateurs ONTAP peuvent link:../authentication/configure-cisco-duo-mfa-task.html["Ajoutez l'authentification à deux facteurs Cisco Duo au processus d'authentification SSH"] renforcer la sécurité de connexion. Lors de la première connexion après l'activation de l'authentification Cisco Duo, les utilisateurs devront enregistrer un périphérique servant d'authentificateur pour les sessions SSH.
* À partir d'ONTAP 9.15.1, les administrateurs peuvent link:../authentication/dynamic-authorization-overview.html["Configurer l'autorisation dynamique"] fournir une authentification adaptative supplémentaire aux utilisateurs SSH en fonction du score de confiance de l'utilisateur.
* À partir de ONTAP 9.19.1, les administrateurs peuvent utiliser la link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=security+ssh+publickey+show["security ssh publickey show -vserver  -host-key-algorithm  command"^] pour afficher l’empreinte de la clé hôte ONTAP. Cela permet aux administrateurs de vérifier que cette empreinte correspond à celle stockée dans ONTAP.
