...
+
Le `-application` paramètre des[`security login` commandes spécifie la méthode d'accès pour un compte utilisateur. Pour en savoir plus sur la commande link:https://docs.NetApp.com/US-en/ONTAP-cli/Security-login-create.html#description[`security login`], consultez le document de référence de la commande ONTAP.
