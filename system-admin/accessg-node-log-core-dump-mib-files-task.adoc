...
summary: 'Le service web Service Processor Infrastructure est activé par défaut pour permettre à un navigateur web d"accéder aux fichiers journaux, aux fichiers de vidage mémoire et aux fichiers MIB d"un nœud du cluster. Les fichiers restent accessibles même lorsque le nœud est hors service, à condition que le nœud soit pris en charge par son partenaire.' 
---
