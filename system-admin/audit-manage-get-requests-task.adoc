...
summary: 'Par défaut, les requêtes SET sont consignées, contrairement aux requêtes GET. Cependant, vous pouvez utiliser la commande settings pour contrôler si les requêtes GET envoyées depuis ONTAP HTML, l"interface de ligne de commandes ONTAP ou depuis les API ONTAP sont consignées dans le fichier.' 
---
...
[role="lead"]
Les requêtes SET sont consignées par défaut et ne peuvent pas être désactivées, tandis que les requêtes GET ne sont pas consignées par défaut. Cependant, vous pouvez contrôler si les requêtes GET envoyées depuis ONTAP HTML (`-httpget`), l'interface de ligne de commande ONTAP (`-cliget`), ou depuis les API ONTAP (`-ontapiget`) sont consignées dans le fichier.
