...
summary: 'Le serveur de gestion de cluster, également appelé adminSVM, est une implémentation spécialisée de machine virtuelle de stockage qui présente le cluster comme une entité unique et gérable. Outre son rôle de domaine administratif de plus haut niveau, le serveur de gestion de cluster possède des ressources qui n"appartiennent pas logiquement à une SVM de données.' 
---
