...
Apprenez-en davantage sur les options de commande  `help` disponibles pour SP et BMC dans link:online-help-at-sp-bmc-cli-task.html["Consultez les informations d'aide sur l'interface de ligne de commandes ONTAP SP/BMC"].

En savoir plus sur l’interface de ligne de commande ONTAP BMC correspondante dans link:bmc-cli-commands-reference.html["Commandes CLI prises en charge pour le contrôleur de gestion de la carte mère ONTAP"].
