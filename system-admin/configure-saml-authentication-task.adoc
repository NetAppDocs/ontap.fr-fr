...
summary: 'À partir d"ONTAP 9.3, vous pouvez configurer l"authentification Security Assertion Markup Language pour les services web.' 
---
...
NOTE: Certaines URL contiennent le point d'interrogation (?). Le point d'interrogation active l'aide active de la ligne de commandes ONTAP. Pour saisir une URL contenant un point d'interrogation, vous devez d'abord désactiver l'aide active avec la commande `set -active-help false`. L'aide active peut ensuite être réactivée avec la commande `set -active-help true`. Pour en savoir plus, consultez link:https://docs.netapp.com/us-en/ontap-cli/set.html["Référence de commande ONTAP"^].
