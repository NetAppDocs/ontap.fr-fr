...
Les options de Nodeshell prises en charge dans le clustershell sont accessibles via : `vserver options clustershell` commande. Pour afficher ces options, vous pouvez interroger l'interface de ligne de commande clustershell avec `vserver options -vserver _nodename_or_clustername_ -option-name ?`
