...
summary: 'Pour permettre à un serveur SSL d"authentifier le cluster ou la machine virtuelle de stockage en tant que client SSL, vous installez un certificat numérique de type client sur le cluster ou la SVM. Ensuite, vous fournissez le certificat client-ca à l"administrateur du serveur SSL pour installation sur le serveur.' 
---
