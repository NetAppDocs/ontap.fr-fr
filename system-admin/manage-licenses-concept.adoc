...
** Utilisation des fonctions associées au package de licences
Si le processus d'évaluation détermine que le cluster présente un risque d'autorisation de licence, le résultat de la commande suggère également une action corrective.
...
[NOTE]
====
Remarque : ONTAP 9.10.1 prend également en charge les clés de licence à 28 caractères via System Manager ou l'interface de ligne de commandes. Cependant, si une licence NLF est installée pour une fonction, vous ne pouvez pas installer une clé de licence à 28 caractères sur la licence NLF pour la même fonction. Pour plus d’informations sur l’installation des NLF ou des clés de licence à l’aide de System Manager, reportez-vous à la section « Activer les nouvelles fonctionnalités ».
