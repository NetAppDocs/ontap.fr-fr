...
Chaque nœud du cluster participe à un protocole de vote qui élit un nœud primaire ; chaque nœud restant est un nœud secondaire. Le nœud primaire est responsable de la synchronisation des informations au sein du cluster. Lorsque le quorum est formé, il est maintenu par un vote continu. Si le nœud primaire se déconnecte et que le cluster est toujours en quorum, un nouveau nœud primaire est élu par les nœuds qui restent en ligne.
