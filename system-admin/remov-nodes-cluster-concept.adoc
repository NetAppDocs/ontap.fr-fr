...
IMPORTANT: Vous ne devez pas effectuer d'opérations telles que `cluster remove-node`, `cluster unjoin`et `node rename` lorsqu'une mise à niveau automatique de ONTAP est en cours.
