...
----
+
Le nœud principal est le nœud qui héberge des processus tels que  `mgmt`,  `vldb`,  `vifmgr`,  `bcomd` et  `crs`.

. Si le nœud que vous supprimez est le nœud principal actuel, permettez à un autre nœud du cluster d'être élu comme nœud principal :
+
.. Rendre le nœud principal actuel inéligible à participer au cluster :
+
[source, cli]
----
...
----
+
Le nœud est considéré comme défaillant jusqu'à ce que son éligibilité soit rétablie. Lorsque le nœud principal devient inéligible, l'un des nœuds restants est élu nouveau nœud principal par le quorum du cluster.
...
Si le nœud partenaire est sélectionné comme nouveau nœud principal, vous devez vérifier s'il détient epsilon avant de le rendre inéligible. Si le nœud partenaire détient epsilon, vous devez déplacer epsilon vers un autre nœud restant dans le cluster avant de le rendre inéligible. Vous effectuez cette opération lorsque vous répétez ces étapes pour supprimer le nœud partenaire.

====
.. Rendez à nouveau l'ancien nœud principal éligible pour participer au cluster :
+
[source, cli]
----
...
+
Si le nœud est le nœud primaire du quorum, le cluster perdra brièvement le quorum puis le retrouvera. Cette perte de quorum est temporaire et n'affecte aucune opération sur les données.
