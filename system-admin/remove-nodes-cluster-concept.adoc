...
NOTE: Si vous effectuez cette étape sur le premier nœud d'une paire HA, vous devez marquer uniquement ce nœud comme inéligible. Ne modifiez pas le statut du partenaire HA. Si le nœud partenaire est sélectionné comme nouveau maître, vous devez vérifier s'il détient epsilon avant de le rendre inéligible. Cela se produit lorsque vous répétez ces étapes pour supprimer le nœud partenaire.
