...
| Lorsque le message est envoyé | Où le message est envoyé 
 ...
 a| 
Quotidien (par défaut, envoyé entre 12 h 00 et 1:00 en tant que message de journal)
 a| 
Adresses spécifiées dans `-partner-address`
...
Support technique, si `-support` est défini sur `enable`
 ...
 a| 
Quotidien (par défaut, envoyé entre 12 h 00 et 1:00 comme un message de performance), si le `-perf` le paramètre est défini sur `true`
 a| 
