...
---
= Fonctionnement du vidage avec le déplacement de volume ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Les opérations de sauvegarde et de restauration sur bande, ainsi que le déplacement de volumes, peuvent s'exécuter en parallèle jusqu'à la tentative de basculement final par le système de stockage. Durant la phase de basculement final, aucune nouvelle opération de sauvegarde ou de restauration sur bande n'est autorisée sur le volume en cours de déplacement. Cependant, les opérations en cours se poursuivent jusqu'à leur achèvement.
