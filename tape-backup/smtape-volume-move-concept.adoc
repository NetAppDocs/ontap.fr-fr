...
[role="lead"]
Les opérations de sauvegarde SMTape et les opérations de déplacement de volumes peuvent s'exécuter en parallèle jusqu'à ce que le système de stockage tente la phase finale de bascule. Pendant la phase finale de bascule, aucune nouvelle opération de sauvegarde SMTape ne peut être lancée sur le volume en cours de déplacement. Cependant, les opérations en cours se poursuivent jusqu'à leur achèvement.
