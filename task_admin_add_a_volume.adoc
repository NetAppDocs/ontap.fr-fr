...
---
= Ajoutez un volume avec ONTAP System Manager
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
+
** *Exportation via NFS* : Crée le volume avec la  `default` règles d'export qui accordent aux utilisateurs un accès complet aux données.
...
+
** À partir d' ONTAP 9.12.1, vous pouvez sélectionner *Activer l'instantané (local)* et choisir une stratégie d'instantané plutôt que d'utiliser la stratégie par défaut.
...
NOTE: Vous pouvez également enregistrer les spécifications de ce volume dans un playbook Ansible. Pour plus de détails, consultez link:task_use_ansible_playbooks_add_edit_volumes_luns.html["Utilisez les manuels de vente Ansible pour ajouter ou modifier des volumes ou des LUN"].
