...
---
= Provisionnez un stockage NAS pour NFS et SMB avec ONTAP System Manager
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Vous pouvez créer des volumes FlexVol ou, pour les systèmes de fichiers volumineux avec des exigences de performances élevées, vous pouvez créer des volumes FlexGroup . Voirlink:./flexgroup/create-task.html["Créer des volumes FlexGroup"] .
