...
summary: 'Créer des LUN pour fournir le stockage d"un hôte ESXi à l"aide du protocole SAN FC ou iSCSI avec ONTAP System Manager (9.7 et versions ultérieures). Les LUN apparaissent comme des disques vers l"hôte ESXi.' 
---
= Provisionnement du stockage SAN pour les datastores VMware
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
.System Manager
--
Créer des LUN pour fournir le stockage avec System Manager pour ONTAP 9.7 ou version ultérieure

Les LUN apparaissent comme des disques vers l'hôte ESXi.

Pour configurer l'accès au protocole SAN pour les hôtes ESXi dans les datastores à l'aide de System Manager Classic (pour ONTAP 9.7 et versions antérieures), consultez les rubriques suivantes :

* https://docs.netapp.com/us-en/ontap-sm-classic/fc-config-esxi/index.html["Présentation de la configuration FC pour ESXi à l'aide de VSC"^]
* https://docs.netapp.com/us-en/ontap-sm-classic/iscsi-config-esxi/index.html["Présentation de la configuration iSCSI pour ESXi à l'aide de VSC"^]
...
. Utilisez Virtual Storage Console (VSC) pour VMware vSphere pour détecter et initialiser le LUN.
. Vérifiez que les hôtes ESXi sont capables d'écrire et de lire les données sur la LUN.


--
.CLI
--
Créez des LUN à fournir du stockage avec l'interface de ligne de commandes de ONTAP.
