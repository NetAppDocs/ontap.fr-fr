...
---
= Effectuez une mise à niveau ONTAP automatisée et sans interruption
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Lors d'une mise à niveau automatisée non perturbatrice (ANDU), ONTAP installe automatiquement l’image ONTAP cible sur chaque nœud, vérifie que le cluster peut être mis à niveau avec succès, puis exécute soit une xref:concept_upgrade_methods.html[mise à niveau par lot ou déploiement] en arrière-plan en fonction du nombre de nœuds dans le cluster.
...
NOTE: Si vous effectuez une mise à niveau vers ONTAP 9.15.1 ou une version ultérieure via la console NetApp , suivez les instructionslink:https://docs.netapp.com/us-en/console-software-updates/get-started/software-updates.html["procédure de mise à niveau dans la documentation de la console NetApp"^] .


IMPORTANT: Modifier le paramètre de l’ `storage failover modify-auto-giveback`option de commande avant le début d’une ANDU n’a aucun impact sur le processus de mise à niveau. Le processus ANDU ignore toute valeur prédéfinie pour cette option lors de la prise de contrôle/restitution requise pour la mise à jour. Par exemple, définir  `-autogiveback`sur false avant de commencer l’ANDU n’interrompt pas la mise à niveau automatique avant la restitution. En savoir plus sur  `storage failover modify-auto-giveback`dans la link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=storage+failover+modify-auto-giveback["Référence de commande ONTAP"^].
...
* Facultativement, link:../update/pre-stage-system-firmware.html["préchargez le micrologiciel système pour une mise à niveau avec un seul redémarrage"].
+
À partir d'ONTAP 9.17.1, si une version plus récente du BIOS ou du firmware LOADER est déjà présente sur le périphérique de démarrage d'un nœud avant que vous n'effectuiez une mise à niveau ANDU, le processus de mise à niveau conserve le firmware plus récent au lieu de l'écraser avec la version intégrée à l'image ONTAP. Cela permet à la fois la mise à niveau du logiciel ONTAP et la mise à jour du firmware de s'effectuer en un seul cycle de redémarrage.
...
+
Pour le FTP anonyme, vous devez spécifier l'URL au format \ftp://anonymous@ftpserver.
...
|===
.. Sélectionnez *Valider* pour exécuter les vérifications de validation préalables à la mise à niveau.
+
...
+
Une liste complète des vérifications préalables à la mise à niveau automatisée s'affiche, ainsi que les erreurs ou avertissements à corriger avant de lancer le processus. Vous devez résoudre toutes les erreurs avant de procéder à la mise à niveau. Il est bonne pratique de résoudre également les avertissements.
...
====
.Et la suite ?
Si vous avez préconfiguré votre firmware, utilisez la commande  `system node firmware bootmedia show` pour vérifier que la version de firmware prévue est active sur chaque nœud.
