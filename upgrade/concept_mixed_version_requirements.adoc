...
|===
* À partir de ONTAP 9.8, un cluster ONTAP à version mixte peut couvrir un maximum de 5 versions majeures. Par exemple, un cluster ONTAP à version mixte peut avoir des nœuds exécutant ONTAP 9.8 et 9.12.1, ou des nœuds exécutant ONTAP 9.9.1 et 9.13.1. Un cluster ONTAP à versions mixtes avec des nœuds exécutant ONTAP 9.8 et 9.13.1 ne serait pas pris en charge, car ce cluster à versions mixtes couvre 6 versions principales.
...
+
Par exemple, si vous utilisez un système FAS8000 exécutant ONTAP 9.5 et que vous ajoutez une nouvelle plateforme C-Series exécutant ONTAP 9.12.1, vous devez mettre à niveau votre système FAS8000 vers ONTAP 9.8 (qui correspond à la version ONTAP maximale prise en charge).
...
. link:https://docs.netapp.com/us-en/ontap-systems-upgrade/upgrade/upgrade-create-aggregate-move-volumes.html["Migration des données"] À partir de tous les nœuds qui ne peuvent pas être mis à niveau vers un nœud exécutant la version ONTAP supérieure.
