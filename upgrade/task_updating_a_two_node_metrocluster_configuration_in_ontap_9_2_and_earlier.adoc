...
---
= Mettez à niveau une configuration MetroCluster ONTAP à deux nœuds
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Si vous disposez d'une configuration MetroCluster à deux nœuds exécutant ONTAP 9.3 ou une version ultérieure, utilisez les étapes de l'interface de ligne de commande (CLI) dans link:automated-upgrade-task.html["Installez l'image ONTAP avec la mise à niveau ONTAP automatisée sans interruption"].
