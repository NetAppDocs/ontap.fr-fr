...
---
= Modifier le mode d'efficacité du volume ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
À partir d' ONTAP 9.10.1, les modes d'efficacité de stockage au niveau du volume _default_ et _efficient_ ne sont pris en charge que pour les systèmes AFF . Ces modes offrent un choix entre la compression de fichiers (par défaut), qui est le mode par défaut lors de la création de nouveaux volumes AFF , ou l'efficacité de stockage sensible à la température (efficace), qui active l'efficacité de stockage sensible à la température (TSSE).


NOTE: TSSE n'est pris en charge que sur les volumes à provisionnement fin. link:enable-temperature-sensitive-efficiency-concept.html["Apprenez-en davantage sur le TSSE"].
