...
* À partir d' ONTAP 9.18.1, vous pouvez attribuer des politiques QoS à des objets contenus dans d'autres objets qui ont des politiques QoS, comme un volume contenu dans une SVM.  Lorsque vous utilisez des politiques QoS imbriquées, c'est la politique la plus restrictive qui est appliquée.
+
Dans les versions antérieures à ONTAP 9.18.1, vous pouvez attribuer des politiques QoS à des objets contenus dans d'autres objets dotés de politiques QoS, mais ces objets sont suivis séparément par la QoS et n'interagissent pas.  Par exemple, si vous avez une SVM avec un plafond de 50 IOPS et un FlexVol volume avec un plafond de 100 IOPS, le FlexVol volume est limité par le plafond de 100 IOPS, mais n'est pas comptabilisé par la QoS dans la charge de travail de la SVM.
