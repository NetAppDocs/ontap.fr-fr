...
summary: Activez la compaction des données en ligne sur les systèmes FAS avec des agrégats Flash Pool ou des agrégats HDD au niveau du volume ou de l’agrégat. 
---
...
[role="lead"]
Vous pouvez activer la compaction des données en ligne sur les systèmes FAS avec des agrégats Flash Pool (hybrides) ou des agrégats HDD au niveau du volume à l'aide de la  `volume efficiency` commande shell du cluster. La compaction des données est désactivée par défaut pour les volumes créés sur les systèmes FAS. En savoir plus sur  `volume efficiency` dans la link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=volume+efficiency["Référence de commande ONTAP"^].
