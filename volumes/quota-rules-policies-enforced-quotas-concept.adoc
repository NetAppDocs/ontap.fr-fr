...
summary: Les quotas sont définis dans des règles de quota spécifiques aux volumes FlexVol. Ces règles de quota sont regroupées dans une politique de quota pour une machine virtuelle de stockage et activées sur chaque volume de la SVM. 
---
= Découvrez les quotas ONTAP, les règles de quotas et les règles d'efficacité ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
