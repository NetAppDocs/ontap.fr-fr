...
summary: 'Sur les clients UNIX, l"utilisateur root est soumis aux quotas d"arborescence, mais pas aux quotas d"utilisateur ni de groupe. Cela permet à l"utilisateur root d"effectuer des actions au nom d"autres utilisateurs qui seraient autrement empêchées par un quota.' 
---
= Découvrez comment les quotas ONTAP sont appliqués à l'utilisateur racine
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
